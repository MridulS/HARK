...
try:
    ...
    from HARK.SolvingMicroDSOPs.Calibration import (
        EstimationParameters as Params,
    )  # Parameters for the consumer type and the estimation
    from HARK.SolvingMicroDSOPs.Calibration import (
        SetupSCFdata as Data,
...
class TempConsumerType(Model.IndShockConsumerType):
    ...
    def simBirth(self, which_agents):
        ...
        self.aNrmNow[which_agents] = self.aNrmInit[
            which_agents
...
EstimationAgent(
    T_sim=EstimationAgent.T_cycle + 1
...
def calculateStandardErrorsByBootstrap(initial_estimate, N, seed=0, verbose=False):
    ...
    for n in range(N):
        ...
        bootstrap_data = (
            bootstrapSampleFromData(Data.scf_data_array, seed=seed_list[n])
        ).T
...
def main(
    ...
    if estimate_model:
        ...
        model_estimate = minimizeNelderMead(
