...
class CobbDouglasEconomy(Market):
    ...
    def makeAggShkDstn(self):
        ...
        None
        """
        self.TranShkAggDstn = approxMeanOneLognormal(
            sigma=self.TranShkAggStd, N=self.TranShkAggCount
        )
        self.PermShkAggDstn = approxMeanOneLognormal(
            sigma=self.PermShkAggStd, N=self.PermShkAggCount
        )
    ...
    def calcRandW(self, aLvlNow, pLvlNow):
        ...
        AggVarsNow = CobbDouglasAggVars(
            MaggNow, AaggPrev, KtoLnow, RfreeNow, wRteNow, PermShkAggNow, TranShkAggNow
        )
        return AggVarsNow
...
class SmallOpenEconomy(Market):
    ...
    def getAggShocks(self):
        ...
        # Package the results into an object and return it
        AggVarsNow = CobbDouglasAggVars(
            MaggNow, AaggNow, KtoLnow, RfreeNow, wRteNow, PermShkAggNow, TranShkAggNow
        )
        return AggVarsNow
...
class CobbDouglasMarkovEconomy(CobbDouglasEconomy):
    ...
    def makeAggShkDstn(self):
        ...
        for i in range(StateCount):
            TranShkAggDstn.append(
                approxMeanOneLognormal(
                    sigma=self.TranShkAggStd[i], N=self.TranShkAggCount
                )
            )
            PermShkAggDstn.append(
                approxMeanOneLognormal(
                    sigma=self.PermShkAggStd[i], N=self.PermShkAggCount
                )
            )
    ...
    def makeAggShkHist(self):
        ...
        for i in range(StateCount):
            ...
            EventDraws = drawDiscrete(
                N=sim_periods, P=self.AggShkDstn[i][0], X=Events, seed=0
            )
...
class CobbDouglasAggVars(HARKobject):
    ...
    def __init__(
        self, MaggNow, AaggNow, KtoLnow, RfreeNow, wRteNow, PermShkAggNow, TranShkAggNow
    ):
        """
...
def main():
    ...
    if solve_agg_shocks_micro:
        ...
        for M in AggShockExample.Mgrid.tolist():
            ...
            c_at_this_M = AggShockExample.cFunc[0](
    ...
    if solve_krusell_smith:
        ...
        t_end = clock()
        print(
            "Solving the Krusell-Smith model took " + str(t_end - t_start) + " seconds."
        )
