        ...
        for t in range(self.T_cycle):
            ...
            if N > 0:
                ...
                MedShkNow[these] = self.RNG.permutation(
                    approxLognormal(
                        N, mu=np.log(MedShkAvg) - 0.5 * MedShkStd ** 2, sigma=MedShkStd
                    )[1]
                )
...
class ConsMedShockSolver(ConsGenIncProcessSolver):
    ...
    def defBoroCnst(self, BoroCnstArt):
        ...
        self.mNrmMinNow = (
            0.0
        )  # Needs to exist so as not to break when solution is created
        self.MPCmaxEff = (
            0.0
        )  # Actually might vary by p, but no use formulating as a function
...
def main():
    ...
    t_end = clock()
    print(
        "Solving a medical shocks consumer took " + mystr(t_end - t_start) + " seconds."
    )
    ...
    for p in range(MedicalExample.pLvlGrid[0].size):
        ...
        P = pLvl * np.ones_like(M)
        vP = MedicalExample.solution[0].vPfunc(M_temp, P) ** (
            -1.0 / MedicalExample.CRRA
        )
        plt.plot(M_temp, vP)
