...
class ConsPerfForesightSolver(object):
    ...
    def defValueFuncs(self):
        ...
        vFuncNvrs = LinearInterp(
            np.array([self.mNrmMin, self.mNrmMin + 1.0]), np.array([0.0, MPCnvrs])
        )
...
def solvePerfForesight(solution_next, DiscFac, LivPrb, CRRA, Rfree, PermGroFac):
    ...
    solver = ConsPerfForesightSolver(
        solution_next, DiscFac, LivPrb, CRRA, Rfree, PermGroFac
    )
...
class ConsIndShockSetup(ConsPerfForesightSolver):
    ...
    def assignParameters(
        ...
        ConsPerfForesightSolver.assignParameters(
            self, solution_next, DiscFac, LivPrb, CRRA, Rfree, PermGroFac
        )
...
class ConsIndShockSolverBasic(ConsIndShockSetup):
    ...
    def makeLinearcFunc(self, mNrm, cNrm):
        ...
        """
        cFuncUnc = LinearInterp(
            mNrm, cNrm, self.MPCminNow * self.hNrmNow, self.MPCminNow
        )
        return cFuncUnc
...
class ConsKinkedRsolver(ConsIndShockSolver):
    ...
    def __init__(
        ...
        assert (
            CubicBool == False
        ), "KinkedR will only work with linear interpolation (for now)"
        assert (
            Rboro >= Rsave
...
class PerfForesightConsumerType(AgentType):
    ...
    def simDeath(self):
        ...
        DeathShks = drawUniform(
            N=self.AgentCount, seed=self.RNG.randint(0, 2 ** 31 - 1)
        )
    ...
    def checkConditions(
        self, verbose=False, verbose_reference=False, public_call=False
    ):
        """
        ...
        RIF = (
            self.LivPrb[0] * (self.Rfree * self.DiscFac) ** (1 / self.CRRA)
        ) / self.Rfree
        if RIF < 1:
            if public_call:
                print(
            ...
            violated = True
            print(
                "The given type violates the Return Impatience Condition with the supplied parameter values; the factor is %1.5f "
                % (RIF)
            )
        ...
        if AIF < 1:
            if public_call:
                print(
                    "The absolute impatience factor value for the supplied parameter values satisfies the absolute impatience condition."
                )
        else:
            print(
                "The given type violates the absolute impatience condition with the supplied parameter values; the AIF is %1.5f "
                % (AIF)
            )
            if verbose:
                violated = True
        ...
        if FHWF < 1:
            ...
            print(
                "The given type violates the finite human wealth condition; the finite human wealth factor value %2.5f "
                % (FHWF)
            )
            violated = True
...
class IndShockConsumerType(PerfForesightConsumerType):
    ...
    def updateIncomeProcess(self):
        ...
        self.timeFwd()
        IncomeDstn, PermShkDstn, TranShkDstn = constructLognormalIncomeProcessUnemployment(
            self
        )
        self.IncomeDstn = IncomeDstn
    ...
    def getShocks(self):
        ...
        for t in range(self.T_cycle):
            ...
            if N > 0:
                ...
                PermShkNow[these] = (
                    IncomeDstnNow[1][EventDraws] * PermGroFacNow
    ...
    def calcBoundingValues(self):
        ...
        if BoroCnstNat < self.BoroCnstArt:
            MPCmax = (
                1.0
            )  # if natural borrowing constraint is overridden by artificial one, MPCmax is 1
        else:
    ...
    def makeEulerErrorFunc(self, mMax=100, approx_inc_dstn=True):
        ...
        if approx_inc_dstn:
            ...
            TranShkDstn = approxMeanOneLognormal(
                N=200,
    ...
    def checkConditions(self, verbose=False, public_call=True):
        ...
        None
        """
        violated = PerfForesightConsumerType.checkConditions(
            self, verbose=verbose, verbose_reference=False
        )
        ...
        if GIF < 1:
            if public_call:
                print(
                    "The growth impatience factor value for the supplied parameter values satisfies the growth impatience condition."
                )
        else:
            violated = True
            print(
                "The given parameter values violate the growth impatience condition for this consumer type; the GIF is: %2.4f"
                % (GIF)
            )
            if verbose:
        ...
        WRIF = (
            ...
            * (self.Rfree * self.DiscFac) ** (1 / self.CRRA)
        ) / self.Rfree
        if WRIF < 1:
            if public_call:
                print(
            ...
            violated = True
            print(
                "The given type violates the weak return impatience condition with the supplied parameter values.  The WRIF is: %2.4f"
                % (WRIF)
            )
            if verbose:
        ...
        EPermShkValFunc = np.dot(
            self.PermShkDstn[0][0], self.PermShkDstn[0][1] ** (1 - self.CRRA)
        )
        FVAF = (
            self.LivPrb[0]
            * self.DiscFac
            * EPermShkValFunc
            * (self.PermGroFac[0] ** (1 - self.CRRA))
        )
        if FVAF < 1:
            if public_call:
                print(
            ...
            print(
                "The given type violates the finite value of autarky condition with the supplied parameter values. The FVAF is %2.4f"
                % (FVAF)
            )
            violated = True
            if verbose:
...
def constructLognormalIncomeProcessUnemployment(parameters):
    ...
    if T_retire > 0:
        if UnempPrbRet > 0:
            ...
            TranShkValsRet = np.array(
                [IncUnempRet, (1.0 - UnempPrbRet * IncUnempRet) / (1.0 - UnempPrbRet)]
            )
    ...
    for t in range(T_cycle):  # Iterate over all periods, counting forward
        ...
        if T_retire > 0 and t >= T_retire:
            ...
            TranShkDstn_t = approxMeanOneLognormal(
                N=TranShkCount, sigma=TranShkStd[t], tail_N=0
            )
            if UnempPrb > 0:
                TranShkDstn_t = addDiscreteOutcomeConstantMean(
                    TranShkDstn_t, p=UnempPrb, x=IncUnemp
                )
            PermShkDstn_t = approxMeanOneLognormal(
                N=PermShkCount, sigma=PermShkStd[t], tail_N=0
            )
            IncomeDstn.append(
...
def applyFlatIncomeTax(
    IncomeDstn, tax_rate, T_retire, unemployed_indices=[], transitory_index=2
):
    """
...
def main():
    ...
    end_time = clock()
    print(
        "Solving a lifecycle consumer took "
        + mystr(end_time - start_time)
        + " seconds."
    )
    ...
    end_time = clock()
    print(
        "Solving a cyclical consumer took " + mystr(end_time - start_time) + " seconds."
    )
