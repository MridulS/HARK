...
def solveConsGenIncProcess(
    ...
    """
    solver = ConsGenIncProcessSolver(
        solution_next,
        IncomeDstn,
        LivPrb,
        DiscFac,
...
class GenIncProcessConsumerType(IndShockConsumerType):
    ...
    def updatepLvlGrid(self):
        ...
        if self.cycles == 1:
            pLvlNow = drawLognormal(
                self.AgentCount,
                mu=self.pLvlInitMean,
            ...
            for t in range(len(self.PermShkStd)):
                if t > 0:
                    PermShkNow = drawDiscrete(
                        N=self.AgentCount,
                        P=self.PermShkDstn[t - 1][0],
                        X=self.PermShkDstn[t - 1][1],
                        exact_match=False,
                        seed=t,
                    )
        ...
        # Calculate "stationary" distribution in infinite horizon (might vary across periods of cycle)
            ...
            T_long = (
                1000
            )  # Number of periods to simulate to get to "stationary" distribution
            pLvlNow = drawLognormal(
                self.AgentCount,
                mu=self.pLvlInitMean,
                sigma=self.pLvlInitStd,
            ...
            for t in range(T_long):
                ...
                pLvlNow[who_dies] = drawLognormal(
                    np.sum(who_dies),
                    mu=self.pLvlInitMean,
                    sigma=self.pLvlInitStd,
                    seed=t + 92615,
                ...
                for j in range(self.T_cycle):  # Update persistent income
                    these = t_cycle == j
                    PermShkTemp = drawDiscrete(
                        N=np.sum(these),
                        P=self.PermShkDstn[j][0],
                        X=self.PermShkDstn[j][1],
                        exact_match=False,
                        seed=t + 13 * j,
                    )
