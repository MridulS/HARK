...
class TractableConsumerSolution(Solution):
    ...
    def __init__(
        self, mNrm_list=[], cNrm_list=[], MPC_list=[], cFunc_U=NullFunc, cFunc=NullFunc
    ):
        """
...
class TractableConsumerType(AgentType):
    def __init__(self, cycles=0, time_flow=False, **kwds):
        ...
        AgentType.__init__(
            self, cycles=cycles, time_flow=time_flow, pseudo_terminal=True, **kwds
        )
    ...
    def simBirth(self, which_agents):
        ...
        self.aLvlNow[which_agents] = drawLognormal(
            N,
    ...
    def getShocks(self):
        ...
        newly_unemployed = drawBernoulli(
            N, p=self.UnempPrb, seed=self.RNG.randint(0, 2 ** 31 - 1)
        )
...
def main():
    ...
    plotFuncs(
        [ExampleType.solution[0].cFunc, ExampleType.solution[0].cFunc_U], 0, m_upper
    )
    ...
    init_consumer_objects = {
        ...
        "PermGroFac": [
            np.array(
                2
                * [base_primitives["PermGroFac"] / (1.0 - base_primitives["UnempPrb"])]
            )
    ...
    print(
        'Solving the same model "the long way" took '
        + str(t_end - t_start)
        + " seconds."
    )
