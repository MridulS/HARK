...
class MarkovConsumerType(IndShockConsumerType):
    ...
    def checkMarkovInputs(self):
        ...
        for MrkvArray_t in self.MrkvArray:
            assert MrkvArray_t.shape == (
                StateCount,
                StateCount,
            ), "MrkvArray not the right shape!"
        for LivPrb_t in self.LivPrb:
            assert LivPrb_t.shape == (
                StateCount,
            ), "Array in LivPrb is not the right shape!"
        for PermGroFac_t in self.LivPrb:
            assert PermGroFac_t.shape == (
                StateCount,
            ), "Array in PermGroFac is not the right shape!"
        ...
        for IncomeDstn_t in self.IncomeDstn:
            assert (
                len(IncomeDstn_t) == StateCount
            ), "List in IncomeDstn is not the right length!"
    ...
    def getShocks(self):
        ...
        if self.global_markov:
            ...
            base_draws = self.RNG.permutation(
                np.arange(self.AgentCount, dtype=float) / self.AgentCount
                + 1.0 / (2 * self.AgentCount)
            )
        newborn = (
            self.t_age == 0
...
def main():
    ...
    print("Consumption functions for each discrete state:")
    mNrmMin = np.min(
        [ImmunityExample.solution[0].mNrmMin[j] for j in range(StateCount)]
    )
