...
class FluctuationsOneAssetIOUs:
    ...
    def StateReduc(self):
        ...
        Xss = np.vstack(
            (
                np.sum(self.joint_distr.copy(), axis=1),
                np.transpose(np.sum(self.joint_distr.copy(), axis=0)),
                np.log(self.par["RB"]),
                0,
            )
        )
        Yss = np.vstack(
            (
                invmutil(
                    np.reshape(
                        self.mutil_c.copy(),
                        (np.product(self.mutil_c.shape), 1),
                        order="F",
                    )
                ),
                np.log(self.par["PI"]),
                np.log(self.targets["Y"]),
                np.log(self.targets["W"]),
                np.log(self.targets["PROFITS"]),
                np.log(self.targets["N"]),
                self.targets["B"],
                self.targets["G"],
            )
        )
        ...
        Tm = np.cos(
            pi
            * np.arange(0, maxlevel, 1)[np.newaxis].T
            * (
                np.linspace(
                    0.5 / self.mpar["nm"] / 2,
        ...
        for j1 in range(0, max(np.shape(self.grid["h"])) - 1):
            for j3 in range(0, max(np.shape(self.grid["m"]))):
                if j1 + j3 < self.mpar["maxdim"] - 2:
                    TT1, TT3 = np.meshgrid(
                        Tm[:, j3],
                        np.vstack((Th[:, j1][np.newaxis].T, 0.0)),
                        indexing="ij",
                    )
                    Poly.append(
                        (TT1.flatten(order="F") * TT3.flatten(order="F"))[np.newaxis].T
                    )
        ...
        for j2 in range(0, max(np.shape(self.grid["m"]))):
            if j2 < self.mpar["maxdim"] - 2:
                TT1, TT3 = np.meshgrid(
                    Tm[:, j2],
                    np.vstack(
                        (np.zeros(max(np.shape(self.grid["h"])) - 1)[np.newaxis].T, 1)
                    ),
                    indexing="ij",
                )
                Poly.append(
        ...
        Gamma = np.zeros(
            (self.mpar["nm"] + self.mpar["nh"], self.mpar["nm"] + self.mpar["nh"] - 3)
        )
        ...
        for j in range(0, self.mpar["nh"] - 2):
            Gamma[
                bb + np.asarray(range(0, self.mpar["nh"] - 1)), bb + j - 1
            ] = -np.squeeze(Xss[bb + np.asarray(range(0, self.mpar["nh"] - 1))])
            Gamma[bb + j, bb - 1 + j] = 1 - Xss[bb + j]
            Gamma[bb + j, bb - 1 + j] = Gamma[bb + j, bb - 1 + j] - sum(
                Gamma[bb + np.asarray(range(0, self.mpar["nh"] - 1)), bb - 1 + j]
            )
        ...
        InvGamma[
            (n2[1] + 2 + 0) : (n2[1] + 2 + n1[0]), (n2[1] + 2 + 0) : (n2[1] + 2 + n1[1])
        ] = InvCheb.T

        Gamma_control[
            (1 * n1[0] + 0) : (1 * n1[0] + oc), (1 * n1[1] + 0) : (1 * n1[1] + oc)
        ] = np.eye(oc)
        InvGamma[
            (n2[1] + 1 * n1[0] + 2 + 0) : (n2[1] + 1 * n1[0] + 2 + oc),
            (n2[1] + 1 * n1[1] + 2 + 0) : (n2[1] + 1 * n1[1] + 2 + oc),
        ] = np.eye(oc)
...
def plot_IRF(
    mpar, par, gx, hx, joint_distr, Gamma_state, grid, targets, os, oc, Output
):
    ...
    IRF_distr = (
        Gamma_state * IRF_state_sparse[: mpar["numstates"] - 2, : mpar["maxlag"]]
    )
    ...
    IRF_H = (
        100
        * grid["h"][:-1]
        * IRF_distr[mpar["nm"] : mpar["nm"] + mpar["nh"] - 1, 1:]
        / par["H"]
    )
...
def Fsys(
    ...
    Control = np.multiply(
        ControlSS.copy(), (1 + Gamma_control.copy().dot(Control_sparse.copy()))
    )
    Controlminus = np.multiply(
        ControlSS.copy(), (1 + Gamma_control.copy().dot(Controlminus_sparse.copy()))
    )

    Control[-oc:] = ControlSS[-oc:].copy() + Gamma_control[-oc:, :].copy().dot(
        Control_sparse.copy()
    )
    Controlminus[-oc:] = ControlSS[-oc:].copy() + Gamma_control[-oc:, :].copy().dot(
        Controlminus_sparse.copy()
    )
    ...
    marginal_hminus = np.transpose(
        Distributionminus[mpar["nm"] : mpar["nm"] + mpar["nh"]].copy()
    )
    ...
    cumdist[1:, 1:] = Copula(
        np.squeeze(np.asarray(np.cumsum(marginal_mminus))),
        np.squeeze(np.asarray(np.cumsum(marginal_hminus))),
    ).T
    ...
    RHS[nx + Yind] = (
        TFP * np.asarray(Nminus) ** par["alpha"] * grid["K"] ** (1 - par["alpha"])
    )

    # Wage Rate
    RHS[nx + Wind] = (
        TFP * par["alpha"] * mc * (grid["K"] / np.asarray(Nminus)) ** (1 - par["alpha"])
    )

    # Profits for Enterpreneurs
    RHS[nx + Profitind] = (1 - mc) * Yminus - Yminus * (1 / (1 - par["mu"])) / par[
        "kappa"
    ] / 2 * np.log(PIminus) ** 2
    ...
    WW = (
        par["gamma"]
        / (1 + par["gamma"])
        * (np.asarray(Nminus) / Hminus)
        * np.asarray(Wminus)
        * np.ones((mpar["nm"], mpar["nh"]))
    )
    WW[:, -1] = Profitminus * par["profitshare"]

    ## Incomes (grids)
    inclabor = par["tau"] * WW.copy() * meshes["h"].copy()
    incmoney = np.multiply(
        meshes["m"].copy(),
        (RBminus / PIminus + (meshes["m"] < 0) * par["borrwedge"] / PIminus),
    )
    ...
    EVm = np.reshape(
        np.reshape(
            np.multiply(RBaux.flatten().T.copy(), mutil_c),
            (mpar["nm"], mpar["nh"]),
            order="F",
    ...
    result_EGM_policyupdate = EGM_policyupdate(
        EVm, PIminus, RBminus, inc, meshes, grid, par, mpar
    )
    ...
    idh = np.kron(range(mpar["nh"]), np.ones((1, mpar["nm"] * mpar["nh"]))).astype(
        np.int64
    )

    indexl1 = np.ravel_multi_index(
        [idm.flatten(order="F"), idh.flatten(order="F")],
        (mpar["nm"], mpar["nh"]),
        order="F",
    )
    indexl2 = np.ravel_multi_index(
        [idm.flatten(order="F") + 1, idh.flatten(order="F")],
        (mpar["nm"], mpar["nh"]),
        order="F",
    )
    ...
    H = sp.coo_matrix(
        (
            ...
            (
                np.squeeze(rowindex),
                np.hstack(
                    (np.squeeze(np.asarray(indexl1)), np.squeeze(np.asarray(indexl2)))
                ),
            ),
        ),
    ...
    if par["tau"] < 1:
        RHS[nx + Gind] = targets["G"] * np.exp(
            -par["gamma_b"] * np.log(Bminus / targets["B"])
            - par["gamma_pi"] * np.log(PIminus / par["PI"])
        )
    ...
    ## Difference
    Difference = InvGamma.dot(
        (LHS - RHS) / np.vstack((np.ones((nx, 1)), ControlSS[:-oc], np.ones((oc, 1))))
    )
...
def EGM_policyupdate(EVm, PIminus, RBminus, inc, meshes, grid, par, mpar):
    ...
    m_n_aux = m_n_aux.copy() / (
        RBminus / PIminus + (m_n_aux.copy() < 0) * par["borrwedge"] / PIminus
    )

    # Identify binding constraints
    binding_constraints = meshes["m"].copy() < np.tile(
        m_n_aux[0, :].copy(), (mpar["nm"], 1)
    )
    ...
    if (
        np.sum(
            np.abs(np.diff(np.sign(np.diff(m_n_aux.copy(), axis=0)), axis=0)), axis=1
        ).max()
        != 0
    ):
    ...
    for hh in range(mpar["nh"]):
        ...
        Savings = interp1d(
            np.squeeze(np.asarray(m_n_aux[:, hh].copy())),
            grid["m"].copy(),
    ...
    c_star[binding_constraints] = np.squeeze(
