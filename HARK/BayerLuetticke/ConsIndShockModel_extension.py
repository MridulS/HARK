...
class IndShockConsumerType_extend(IndShockConsumerType):
    ...
    def DefineDistributionGrid(self, Dist_mGrid=None, Dist_pGrid=None):
        ...
        if self.cycles != 0:
            print(
                "Distributional methods presently only work for perpetual youth agents (cycles=0)"
            )
        else:
            if Dist_mGrid == None:
            ...
            if Dist_pGrid == None:
                ...
                one_sided_grid = makeGridExpMult(
                    1.0 + 1e-3, np.exp(max_p), num_points, 2
                )
        ...
        for i in range(len(Dist_mGrid)):
            for j in range(len(Dist_pGrid)):
                ...
                TranMatrix[:, i * len(Dist_pGrid) + j] = (
                    LivPrb * self.JumpToGrid(mNext_ij, pNext_ij, ShockProbs)
    ...
    def JumpToGrid(self, m_vals, perm_vals, probs):
        ...
        for i in range(len(m_vals)):
            if mIndex[i] == -1:
                ...
                mlowerWeight = (self.Dist_mGrid[mupperIndex] - m_vals[i]) / (
                    self.Dist_mGrid[mupperIndex] - self.Dist_mGrid[mlowerIndex]
                )
            ...
            if pIndex[i] == -1:
                ...
                plowerWeight = (self.Dist_pGrid[pupperIndex] - perm_vals[i]) / (
                    self.Dist_pGrid[pupperIndex] - self.Dist_pGrid[plowerIndex]
                )
    ...
    def CalcErgodicDist(self):
        ...
        self.ergodic_distr = ergodic_distr.reshape(
...
if __name__ == "__main__":
    ...
    IndShock_extendExample = IndShockConsumerType_extend(
    ...
    end_time = clock()
    print(
        "Calculating the transition matrix took "
        + mystr(end_time - start_time)
        + " seconds."
    )
    start_time = clock()
    ...
    end_time = clock()
    print(
        "Calculating the ergodic distribution took "
        + mystr(end_time - start_time)
        + " seconds."
    )
