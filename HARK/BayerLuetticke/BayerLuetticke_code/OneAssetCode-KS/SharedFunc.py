...
def MakeGrid(mpar, grid):
    ...
    grid["m"] = (
        np.exp(
            np.exp(np.linspace(0.0, np.log(np.log(m_max - m_min + 1) + 1), mpar["nm"]))
            - 1
        )
        - 1
        + m_min
    ...
    grid["m"][np.abs(grid["m"]) == np.min(grid["m"])] = 0.0
...
def Tauchen(rho, N, sigma, mue, types):
    ...
    if types == "importance":  # Importance sampling
        ...
        pijfunc = lambda x, bound1, bound2: norm.pdf(x) * (
            norm.cdf((bound2 - rho * x) / sigma_e)
            - norm.cdf((bound1 - rho * x) / sigma_e)
        )
