...
class SteadyStateOneAssetIOU:
    ...
    def SolveSteadyState(self):
        ...
        labortax = (1.0 - par["tau"]) * W_fc * N + (1.0 - par["tau"]) * Profits_fc
        par["gamma1"] = tgB * (1.0 - par["RB"]) + labortax
    ...
    def PolicyGuess(self, meshes, WW, RBRB, par, mpar):
        ...
        c_guess = (
            inc["labor"].copy() + np.maximum(inc["money"].all(), 0.0) + inc["profits"]
        )
    ...
    def FactorReturns(self, meshes, grid, par, mpar):
        ...
        w = 1.0 / 4.0 * par["alpha"] * mc * (grid["K"] / N) ** (1 - par["alpha"])
