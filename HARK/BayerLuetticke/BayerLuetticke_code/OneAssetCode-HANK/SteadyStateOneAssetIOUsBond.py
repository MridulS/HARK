...
class SteadyStateOneAssetIOUsBond:
    ...
    def SolveSteadyState(self):
        ...
        count = 0
        inc = {
            "labor": 0.9 * np.ones((self.mpar["nm"], self.mpar["nh"]))
        }  # initial labor income for natural borrowing const
        ...
        while np.abs(init) > self.mpar["crit"]:
            ...
            if (
                r + par["borrwedge"] > 0
                and -inc["labor"][0, 0] / (r + par["borrwedge"]) > grid["m_min"]
            ):
                regrid = MakeGrid2(
                    self.mpar,
                    grid,
                    -inc["labor"][0, 0] / (r + par["borrwedge"]) + 10 ** (-11),
                    grid["m_max"],
                )
            else:
            ...
            resultPolicySS = self.PoliciesSS(
                c_guess, grid, inc, RBRB, P_H, self.mpar, par
            )
            ...
            joint_distr = np.reshape(
                joint_distr.copy(), (self.mpar["nm"], self.mpar["nh"]), order="F"
            )
            AggregateSavings = (
                m_star.flatten("F")
                .transpose()
                .dot(joint_distr.flatten("F").transpose())
            )
            ...
            init = rmax - rmin
            print(
                "Starting Iteration for r. Difference remaining:                      "
            )
            print(init)
            r = (rmax + rmin) / 2.0
            par["RB"] = 1.0 + r
        ...
        tax = (1.0 - par["tau"]) * W_fc * N + (1.0 - par["tau"]) * Profits_fc
        ...
        GiniS = np.concatenate((np.array([0.0]), GiniS_aux.copy()), axis=0)
        tgGini = (
            1.0
            - np.sum(Ginipdf * (GiniS.copy()[:-1] + GiniS.copy()[1:]))
            / GiniS.copy()[-1]
        )
        ...
        # Liquid Asset
        mutil_c = 1.0 / (c_guess.copy() ** par["xi"])
    ...
    def JDiteration(self, m_star, P_H, mpar, grid):
        ...
        for hh in range(mpar["nh"]):
            ...
            weight11_aux = 1.0 - Dist_m[:, hh].copy()
        ...
        while (distJD > 10 ** (-14) or countJD < 50) and countJD < 10000:
            ...
            distJD = np.max(
                (
                    np.abs(
                        joint_distr_next.copy().flatten() - joint_distr.copy().flatten()
                    )
                )
            )
    ...
    def PoliciesSS(self, c_guess, grid, inc, RBRB, P, mpar, par):
        ...
        while np.max((distC)) > mpar["crit"]:
            ...
            mutil_c = 1.0 / (
                c_guess.copy() ** par["xi"]
            )  # marginal utility at consumption policy

            aux = np.reshape(
                np.ndarray.transpose(mutil_c.copy(), (1, 0)), (mpar["nh"], mpar["nm"])
            )
            ...
            EMU_aux = (
                ...
                * np.ndarray.transpose(
                    np.reshape(P.copy().dot(aux.copy()), (mpar["nh"], mpar["nm"])),
                    (1, 0),
                )
            )

            c_aux = 1.0 / (EMU_aux.copy() ** (1 / par["xi"]))
    ...
    def EGM(self, grid, inc, money_expense, c_aux, mpar, par):
        ...
        RR = (par["RB"] + (m_star.copy() < 0.0) * par["borrwedge"]) / par["PI"]
        ...
        for hh in range(mpar["nh"]):
            ...
            Savings = interp1d(
                m_star[:, hh].copy(), grid["m"], fill_value="extrapolate"
            )
            m_update[:, hh] = Savings(grid["m"])
            Consumption = interp1d(
                m_star[:, hh].copy(), c_aux[:, hh], fill_value="extrapolate"
            )
    ...
    def PolicyGuess(self, meshes, WW, RBRB, par, mpar):
        ...
        incprofits = 0.0  # lumpsum profits
        ...
        c_guess = inc["labor"].copy() + np.maximum(inc["money"], 0.0) + inc["profits"]
    ...
    def FactorReturns(self, meshes, grid, par, mpar):
        ...
        mc = par["mu"]  # in SS
        N = (par["tau"] * par["alpha"] * grid["K"] ** (1 - par["alpha"]) * mc) ** (
        ...
        NW = par["gamma"] / (1.0 + par["gamma"]) * N / par["H"] * w
