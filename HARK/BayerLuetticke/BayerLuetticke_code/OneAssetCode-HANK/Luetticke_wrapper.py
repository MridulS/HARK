...
if __name__ == "__main__":
    ...
    LuettickeExampleEconomy = LuettickeEconomy(
        FluctuationsOneAssetIOU, agents=[LuettickeExampleAgent]
    )
    # Solve the market
    ...
    if simulate:
        ...
        for t in range(LuettickeExampleAgent.T_sim):
            meanbNow0[t] = np.mean(
                LuettickeExampleAgent.bNow_hist[t, :][
                    LuettickeExampleAgent.incStateNow_hist[t, :] == 0
                ]
            )
            meanbNow1[t] = np.mean(
                LuettickeExampleAgent.bNow_hist[t, :][
                    LuettickeExampleAgent.incStateNow_hist[t, :] == 1
                ]
            )
            meanbNow2[t] = np.mean(
                LuettickeExampleAgent.bNow_hist[t, :][
                    LuettickeExampleAgent.incStateNow_hist[t, :] == 2
                ]
            )
            meanbNow3[t] = np.mean(
                LuettickeExampleAgent.bNow_hist[t, :][
                    LuettickeExampleAgent.incStateNow_hist[t, :] == 3
                ]
            )
