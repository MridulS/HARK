...
class SteadyStateTwoAsset:
    ...
    def SolveSteadyState(self):
        ...
        grid = MakeGridkm(
            self.mpar, grid, grid["k_min"], grid["k_max"], grid["m_min"], grid["m_max"]
        )
        meshes = {}
        meshes["m"], meshes["k"], meshes["h"] = np.meshgrid(
            grid["m"], grid["k"], grid["h"], indexing="ij"
        )
        ...
        ## SS stats
        mesh = {}
        mesh["m"], mesh["k"] = np.meshgrid(
            grid["m"].copy(), grid["k"].copy(), indexing="ij"
        )

        targets = {}
        targets["ShareBorrower"] = np.sum(
            (grid["m"] < 0)
            * np.transpose(np.sum(np.sum(joint_distr.copy(), axis=1), axis=1))
        )
        targets["K"] = np.sum(
            grid["k"].copy() * np.sum(np.sum(joint_distr.copy(), axis=0), axis=1)
        )
        targets["B"] = np.dot(
            grid["m"].copy(), np.sum(np.sum(joint_distr.copy(), axis=1), axis=1)
        )
        grid["K"] = targets["K"]
        grid["B"] = targets["B"]
        ...
        targets["L"] = grid["N"] * np.sum(
            np.dot(grid["h"].copy(), np.sum(np.sum(joint_distr.copy(), axis=0), axis=0))
        )
        ...
        targets["WtH_bnonpos"] = np.sum(
            aux_MK[(mesh["m"] <= 0) * (mesh["k"] > 0)].copy()
        )

        targets["T"] = (1.0 - par["tau"]) * W_fc * grid["N"] + (
            1.0 - par["tau"]
        ) * Profits_fc
        ...
        ## Ginis
        # Net worth Gini
        mplusk = mesh["k"].copy().flatten("F") * par["Q"] + mesh["m"].copy().flatten(
            "F"
        )
        ...
        S = np.concatenate(([0.0], S.copy()))
        targets["GiniW"] = 1.0 - (
            np.sum(moneycapital_pdf.copy() * (S[:-1].copy() + S[1:].copy()).transpose())
            / S[-1]
        )
        ...
        S = np.concatenate(([0.0], S.copy()))
        targets["GiniLI"] = 1.0 - (
            np.sum(liquid_pdf.copy() * (S[:-1].copy() + S[1:].copy())) / S[-1].copy()
        )
        ...
        S = np.concatenate(([0.0], S.copy()))
        targets["GiniIL"] = 1.0 - (
            np.sum(illiquid_pdf.copy() * (S[:-1].copy() + S[1:].copy())) / S[-1].copy()
        )

        ##   MPCs
        meshesm, meshesk, meshesh = np.meshgrid(
            grid["m"], grid["k"], grid["h"], indexing="ij"
        )

        NW = par["gamma"] / (1.0 + par["gamma"]) * (par["N"] / par["H"]) * par["W"]
        ...
        for kk in range(0, self.mpar["nk"]):
            for hh in range(0, self.mpar["nh"]):
                MPC_a_m[:, kk, hh] = (
                    np.gradient(np.squeeze(c_a_guess[:, kk, hh].copy()))
                    / np.gradient(grid["m"].copy()).transpose()
                )
                MPC_n_m[:, kk, hh] = (
                    np.gradient(np.squeeze(c_n_guess[:, kk, hh].copy()))
                    / np.gradient(grid["m"].copy()).transpose()
                )
        ...
        for mm in range(0, self.mpar["nm"]):
            for kk in range(0, self.mpar["nk"]):
                MPC_a_h[mm, kk, :] = (
                    np.gradient(np.squeeze(np.log(c_a_guess[mm, kk, :].copy())))
                    / np.gradient(
                        np.log(WW_h.copy().transpose() * grid_h_aux.copy())
                    ).transpose()
                )
                MPC_n_h[mm, kk, :] = (
                    np.gradient(np.squeeze(np.log(c_n_guess[mm, kk, :].copy())))
                    / np.gradient(
                        np.log(WW_h.copy().transpose() * grid_h_aux.copy())
                    ).transpose()
                )
        ...
        EMPC_h = np.dot(
            joint_distr.copy().flatten("F"),
            (
                par["nu"] * MPC_a_h.copy().flatten("F")
                + (1.0 - par["nu"]) * MPC_n_h.copy().flatten("F")
            ),
        )
        EMPC_m = np.dot(
            joint_distr.copy().flatten("F"),
            (
                par["nu"] * MPC_a_m.copy().flatten("F")
                + (1.0 - par["nu"]) * MPC_n_m.copy().flatten("F")
            ),
        )
        ...
        targets["Insurance_coeff"] = np.concatenate(
            (
                np.concatenate(([[1.0 - EMPC_h]], [[1.0 - EMPC_m]]), axis=1),
                np.concatenate(([[1.0 - EMPC_a_h]], [[1.0 - EMPC_a_m]]), axis=1),
                np.concatenate(([[1.0 - EMPC_n_h]], [[1.0 - EMPC_n_m]]), axis=1),
            ),
            axis=0,
        )
        ...
        RBRB = par["RB"] / par["PI"] + (meshes["m"] < 0) * (
            par["borrwedge"] / par["PI"]
        )

        # Liquid Asset
        mutil_c_n = 1.0 / (
            c_n_guess.copy() ** par["xi"]
        )  # marginal utility at consumption policy no adjustment
        mutil_c_a = 1.0 / (
            c_a_guess.copy() ** par["xi"]
        )  # marginal utility at consumption policy adjustment
        mutil_c = (
            par["nu"] * mutil_c_a.copy() + (1 - par["nu"]) * mutil_c_n.copy()
        )  # Expected marginal utility at consumption policy (w &w/o adjustment)
        ...
        # Capital
        Vk = (
            par["nu"] * (par["R"] + par["Q"]) * mutil_c_a.copy()
            + (1 - par["nu"]) * par["R"] * mutil_c_n.copy()
            + (1 - par["nu"]) * psi_guess.copy()
        )  # Expected marginal utility at consumption policy (w &w/o adjustment)
        Vk = np.reshape(
            np.reshape(
                Vk.copy(),
        ...
        cum_dist = np.cumsum(
            np.cumsum(np.cumsum(joint_distr.copy(), axis=0), axis=1), axis=2
        )
        marginal_m = np.cumsum(
            np.squeeze(np.sum(np.sum(joint_distr.copy(), axis=1), axis=1))
        )
        marginal_k = np.cumsum(
            np.squeeze(np.sum(np.sum(joint_distr.copy(), axis=0), axis=1))
        )
        marginal_h = np.cumsum(
            np.squeeze(np.sum(np.sum(joint_distr.copy(), axis=1), axis=0))
        )

        Cgridm, Cgridk, Cgridh = np.meshgrid(
            marginal_m.copy(), marginal_k.copy(), marginal_h.copy(), indexing="ij"
        )
        Cpoints = np.concatenate(
            (
                [Cgridm.flatten(order="F")],
                [Cgridk.flatten(order="F")],
                [Cgridh.flatten(order="F")],
            ),
            axis=0,
        ).T
    ...
    def JDiteration(
        self, joint_distr, m_n_star, m_a_star, cap_a_star, P_H, par, mpar, grid
    ):
        """
        ...
        idk_n = np.tile(
            np.ones((mpar["nm"], 1)) * np.arange(mpar["nk"]), (1, 1, mpar["nh"])
        )
        ...
        idh = np.kron(
            np.arange(mpar["nh"]), np.ones((1, mpar["nm"] * mpar["nk"] * mpar["nh"]))
        )
        ...
        index11 = np.ravel_multi_index(
            [
                idm_a.flatten(order="F"),
                idk_a.flatten(order="F"),
                idh.flatten(order="F"),
            ],
        ...
        index12 = np.ravel_multi_index(
            [
                idm_a.flatten(order="F"),
                idk_a.flatten(order="F") + 1,
                idh.flatten(order="F"),
            ],
            (mpar["nm"], mpar["nk"], mpar["nh"]),
            order="F",
        )
        index21 = np.ravel_multi_index(
            [
                idm_a.flatten(order="F") + 1,
                idk_a.flatten(order="F"),
                idh.flatten(order="F"),
            ],
            (mpar["nm"], mpar["nk"], mpar["nh"]),
        ...
        index13 = np.ravel_multi_index(
            [
                idm_n.flatten(order="F"),
                idk_n.flatten(order="F"),
                idh.flatten(order="F"),
            ],
            (mpar["nm"], mpar["nk"], mpar["nh"]),
            order="F",
        )
        index23 = np.ravel_multi_index(
            [
                idm_n.flatten(order="F") + 1,
                idk_n.flatten(order="F"),
                idh.flatten(order="F"),
            ],
            (mpar["nm"], mpar["nk"], mpar["nh"]),
        ...
        for hh in range(mpar["nh"]):
            ...
            weight21_aux = Dist_m_a[:, :, hh].copy() * (1.0 - Dist_k[:, :, hh].copy())
            weight11_aux = (1.0 - Dist_m_a[:, :, hh].copy()) * (
                1 - Dist_k[:, :, hh].copy()
            )
            weight22_aux = Dist_m_a[:, :, hh].copy() * Dist_k[:, :, hh].copy()
            weight12_aux = (1.0 - Dist_m_a[:, :, hh].copy()) * Dist_k[:, :, hh].copy()

            weight23_aux = Dist_m_n[:, :, hh].copy()
            weight13_aux = 1.0 - Dist_m_n[:, :, hh].copy()

            # Dimensions (mxk,h',h)
            weight11[:, :, hh] = np.outer(
                weight11_aux.flatten(order="F"), P_H[hh, :].copy()
            )
            weight12[:, :, hh] = np.outer(
                weight12_aux.flatten(order="F"), P_H[hh, :].copy()
            )
            weight21[:, :, hh] = np.outer(
                weight21_aux.flatten(order="F"), P_H[hh, :].copy()
            )
            weight22[:, :, hh] = np.outer(
                weight22_aux.flatten(order="F"), P_H[hh, :].copy()
            )

            weight13[:, :, hh] = np.outer(
                weight13_aux.flatten(order="F"), P_H[hh, :].copy()
            )
            weight23[:, :, hh] = np.outer(
                weight23_aux.flatten(order="F"), P_H[hh, :].copy()
            )
        ...
        rowindex = np.tile(
            range(mpar["nm"] * mpar["nk"] * mpar["nh"]), (1, 4 * mpar["nh"])
        )
        ...
        H_a = sp.coo_matrix(
            (
                ...
                (
                    rowindex.flatten(order="F"),
                    np.concatenate(
                        (
                            index11.flatten(order="F"),
                            index21.flatten(order="F"),
                            index12.flatten(order="F"),
                            index22.flatten(order="F"),
                        )
                    ),
                ),
            ),
        ...
        )  # mu'(h',k'), a without interest

        weight13 = np.ndarray.transpose(weight13.copy(), (0, 2, 1))
        weight23 = np.ndarray.transpose(weight23.copy(), (0, 2, 1))

        rowindex = np.tile(
            range(mpar["nm"] * mpar["nk"] * mpar["nh"]), (1, 2 * mpar["nh"])
        )

        H_n = sp.coo_matrix(
            (
                np.concatenate(
                    (weight13.flatten(order="F"), weight23.flatten(order="F"))
                ),
                (
                    rowindex.flatten(order="F"),
                    np.concatenate(
                        (index13.flatten(order="F"), index23.flatten(order="F"))
                    ),
                ),
            ),
            shape=(
        ...
        H = par["nu"] * H_a.copy() + (1.0 - par["nu"]) * H_n.copy()
        ...
        distJD = np.max(
            (
                np.abs(
                    joint_distr_next.copy().flatten(order="F")
                    - joint_distr.copy().flatten(order="F")
                )
            )
        )
        ...
        while (distJD > 10 ** (-14) or countJD < 50) and countJD < 10000:
            ...
            joint_distr_next = joint_distr.copy().dot(H.copy().todense())
            joint_distr_next = joint_distr_next.copy() / joint_distr_next.copy().sum(
                axis=1
            )
    ...
    def PoliciesSS(
        self, c_a_guess, c_n_guess, psi_guess, grid, inc, RR, RBRB, P, mpar, par, meshes
    ):
        ...
        while max(distC_n, distC_a, distPSI) > mpar["crit"] and count < 100000:
            ...
            aux = np.reshape(
                np.ndarray.transpose(mutil_c.copy(), (2, 0, 1)),
                (mpar["nh"], mpar["nm"] * mpar["nk"]),
                order="F",
            )
            # form expectations
            EMU_aux = par["beta"] * np.ndarray.transpose(
                np.reshape(
                    P.copy().dot(aux.copy()),
                    (mpar["nh"], mpar["nm"], mpar["nk"]),
                    order="F",
            ...
            c_n_aux = 1.0 / (EMU_aux.copy() ** (1.0 / par["xi"]))

            # Take borrowing constraint into account
            results_EGM_Step1_b = self.EGM_Step1_b(
                grid, inc, c_n_aux, mpar, par, meshes
            )
            c_n_new = results_EGM_Step1_b["c_update"].copy()
            m_n_star = results_EGM_Step1_b["m_update"].copy()

            m_n_star[m_n_star.copy() > grid["m"][-1]] = grid["m"][
                -1
            ]  # not extrapolation

            # Step 2: Find for every k on grid some off-grid m*(k')
            m_a_star_aux = self.EGM_Step2_SS(
                mutil_c_n, mutil_c_a, psi_guess, grid, P, RBRB, RR, par, mpar
            )
            m_a_star_aux = m_a_star_aux["mstar"].copy()

            # Step 3: Solve for initial resources / consumption in  adjustment case
            results_EGM_Step3 = self.EGM_Step3(
                EMU_aux, grid, inc, m_a_star_aux, c_n_aux, mpar, par
            )
            ...
            results_EGM_Step4 = self.EGM_Step4(
                cons_list, res_list, mon_list, cap_list, inc, mpar, grid
            )
            ...
            cap_a_star[cap_a_star.copy() > grid["k"][-1]] = grid["k"][
                -1
            ]  # not extrapolation
            m_a_star[m_a_star.copy() > grid["m"][-1]] = grid["m"][
                -1
            ]  # not extrapolation
            ...
            term1 = (
                (par["nu"] * mutil_c_a.copy() * (par["Q"] + RR))
                + ((1.0 - par["nu"]) * mutil_c_n.copy() * RR)
                + (1.0 - par["nu"]) * psi_guess.copy()
            )

            aux = np.reshape(
                np.ndarray.transpose(term1.copy(), (2, 0, 1)),
                (mpar["nh"], mpar["nm"] * mpar["nk"]),
                order="F",
            )
            E_rhs_psi = par["beta"] * np.ndarray.transpose(
                np.reshape(
                    P.copy().dot(aux.copy()),
                    (mpar["nh"], mpar["nm"], mpar["nk"]),
                    order="F",
            ...
            E_rhs_psi = np.reshape(
                E_rhs_psi.copy(), (mpar["nm"], mpar["nk"] * mpar["nh"]), order="F"
            )
            m_n_star = np.reshape(
                m_n_star.copy(), (mpar["nm"], mpar["nk"] * mpar["nh"]), order="F"
            )

            # Interpolation of psi-function at m*_n(m,k)
            index = (
                np.digitize(m_n_star.copy(), grid["m"]) - 1
            ...
            s = (np.asmatrix(m_n_star.copy()) - np.squeeze(grid["m"].T[index])) / step[
                index
            ]  # Distance of optimal policy to next grid point

            aux_index = (
                np.ones((mpar["nm"], 1))
                * np.arange(0, mpar["nk"] * mpar["nh"])
                * mpar["nm"]
            )  # aux for linear indexes
            E_rhs_psi = E_rhs_psi.flatten(order="F").copy()
            aux3 = E_rhs_psi[
                (
                    index.flatten(order="F").copy()
                    + aux_index.flatten(order="F").copy()
                ).astype(int)
            ]  # calculate linear indexes

            psi_new = aux3.copy() + np.squeeze(
                np.asarray(s.flatten(order="F").copy())
            ) * (
                E_rhs_psi[
                    (index.flatten(order="F") + aux_index.flatten(order="F")).astype(
                        int
                    )
                    + 1
                ].copy()
            ...
            psi_new = np.reshape(
                psi_new.copy(), (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
            )
            m_n_star = np.reshape(
                m_n_star.copy(), (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
            )
            distPSI = max(
                (
                    abs(
                        psi_guess.flatten(order="F").copy()
                        - psi_new.flatten(order="F").copy()
                    )
                )
            )
            ...
            distC_n = max(
                (
                    abs(
                        c_n_guess.flatten(order="F").copy()
                        - c_n_new.flatten(order="F").copy()
                    )
                )
            )
            distC_a = max(
                (
                    abs(
                        c_a_guess.flatten(order="F").copy()
                        - c_a_new.flatten(order="F").copy()
                    )
                )
            )
    ...
    def EGM_Step1_b(self, grid, inc, c_n_aux, mpar, par, meshes):
        ...
        m_star_n = (
            c_n_aux.copy() + meshes["m"] - inc["labor"] - inc["rent"] - inc["profits"]
        )
        m_star_n = (m_star_n.copy() < 0) * m_star_n.copy() / (
            (par["RB"] + par["borrwedge"]) / par["PI"]
        ) + (m_star_n.copy() >= 0) * m_star_n.copy() / (par["RB"] / par["PI"])

        # Identify binding constraints
        binding_constraints = meshes["m"] < np.tile(
            m_star_n[0, :, :].copy(), (mpar["nm"], 1, 1)
        )
        ...
        m_star_n = np.reshape(
            m_star_n.copy(), (mpar["nm"], mpar["nk"] * mpar["nh"]), order="F"
        )
        c_n_aux = np.reshape(
            c_n_aux.copy(), (mpar["nm"], mpar["nk"] * mpar["nh"]), order="F"
        )
        ...
        for hh in range(mpar["nk"] * mpar["nh"]):
            ...
            Savings = interp1d(
                m_star_n[:, hh].copy(), grid["m"], fill_value="extrapolate"
            )  # generate savings function a(s,a*)=a'
            m_update[:, hh] = Savings(grid["m"])  # Obtain m'(m,h) by Interpolation
            Consumption = interp1d(
                m_star_n[:, hh].copy(), c_n_aux[:, hh], fill_value="extrapolate"
            )  # generate consumption function c(s,a*(s,a'))
            c_update[:, hh] = Consumption(
                grid["m"]
            )  # Obtain c(m,h) by interpolation (notice this is out of grid, used linear interpolation)

        c_update = np.reshape(c_update, (mpar["nm"], mpar["nk"], mpar["nh"]), order="F")
        m_update = np.reshape(m_update, (mpar["nm"], mpar["nk"], mpar["nh"]), order="F")

        c_update[binding_constraints] = (
            Resource[binding_constraints].copy() - grid["m"].T[0]
        )
    ...
    def EGM_Step2_SS(
        self, mutil_c_n, mutil_c_a, psi_guess, grid, P, RBRB, RR, par, mpar
    ):

        term1 = (
            (par["nu"] * mutil_c_a.copy() * (par["Q"] + RR))
            + ((1 - par["nu"]) * mutil_c_n.copy() * RR)
            + (1 - par["nu"]) * psi_guess.copy()
        )
        aux = np.reshape(
            np.ndarray.transpose(term1.copy(), (2, 0, 1)),
        ...
        term1 = par["beta"] * np.ndarray.transpose(
            np.reshape(
                P.copy().dot(aux.copy()),
                (mpar["nh"], mpar["nm"], mpar["nk"]),
                order="F",
        ...
        aux = np.reshape(
            np.ndarray.transpose(term2.copy(), (2, 0, 1)),
            (mpar["nh"], mpar["nm"] * mpar["nk"]),
            order="F",
        )
        ...
        term2 = par["beta"] * np.ndarray.transpose(
            np.reshape(
                P.copy().dot(aux.copy()),
                (mpar["nh"], mpar["nm"], mpar["nk"]),
                order="F",
        ...
        mstar = np.maximum(
            mstar.copy(), grid["m"].T[0]
        )  # Use non-negativity constraint and monotonicity
        mstar = np.minimum(
            mstar.copy(), grid["m"].T[-1]
    ...
    def EGM_Step3(self, EMU, grid, inc, m_a_star, c_n_aux, mpar, par):
        ...
        # Interpolation of psi-function at m*_n(m,k)
        index = (
            np.digitize(np.asarray(m_a_star.copy()), np.squeeze(grid["m"])) - 1
        )  # find indexes on grid next smallest to optimal policy
        index[m_a_star <= grid["m"].T[0]] = 0  # if below minimum
        index[m_a_star >= grid["m"].T[-1]] = mpar["nm"] - 2  # if above maximum
        step = np.squeeze(np.diff(grid["m"]))  # Stepsize on grid
        s = (np.asmatrix(m_a_star.T) - grid["m"].T[index].T) / step.T[
            index
        ].T  # Distance of optimal policy to next grid point

        aux_index = (
            np.arange(0, (mpar["nk"] * mpar["nh"]), 1) * mpar["nm"]
        )  # aux for linear indexes
        EMU = EMU.flatten(order="F").copy()
        aux3 = EMU[
            index.flatten(order="F") + aux_index.flatten(order="F")
        ].copy()  # calculate linear indexes
        ...
        c_star = 1.0 / (EMU_star.copy() ** (1 / par["xi"]))
        ...
        Resource = (
            c_star.flatten(order="F").copy()
            + m_a_star.flatten(order="F").copy()
            + cap_expenditure.flatten(order="F").copy()
            - auxL.flatten(order="F").copy()
            - auxP
        )
        ...
        for j in range(mpar["nh"]):
            ...
            if m_star_zero[j] > grid["m"].T[0]:
                ...
                res_list.append(
                    np.squeeze(grid["m"].T[log_index]) + c_k_cons.copy() - aux_inc.T[j]
                )
        ...
        for j in range(mpar["nh"]):
            if m_star_zero[j] > grid["m"].T[0]:
                cons_list_1.append(
                    np.vstack(
                        (np.asmatrix(cons_list[j]).T, np.asmatrix(c_star[:, j]).T)
                    )
                )
                res_list_1.append(
                    np.vstack(
                        (np.asmatrix(res_list[j]).T, np.asmatrix(Resource[:, j]).T)
                    )
                )
                mon_list_1.append(
                    np.vstack(
                        (np.asmatrix(mon_list[j]).T, np.asmatrix(m_a_star[:, j]).T)
                    )
                )
                cap_list_1.append(
                    np.vstack((np.asmatrix(cap_list[j]), np.asmatrix(grid["k"]).T))
                )
            else:
    ...
    def EGM_Step4(self, cons_list, res_list, mon_list, cap_list, inc, mpar, grid):
        ...
        Resource_grid = np.reshape(
            inc["capital"] + inc["money"] + inc["rent"],
            (mpar["nm"] * mpar["nk"], mpar["nh"]),
            order="F",
        )
        labor_inc_grid = np.reshape(
            inc["labor"] + inc["profits"],
            (mpar["nm"] * mpar["nk"], mpar["nh"]),
            order="F",
        )
        ...
        for j in range(mpar["nh"]):
            ...
            cons = interp1d(
                np.squeeze(np.asarray(res_list[j].copy())),
                np.squeeze(np.asarray(cons_list[j].copy())),
                fill_value="extrapolate",
            )
            c_a_new[:, j] = cons(Resource_grid[:, j])
            mon = interp1d(
                np.squeeze(np.asarray(res_list[j].copy())),
                np.squeeze(np.asarray(mon_list[j].copy())),
                fill_value="extrapolate",
            )
            m_a_new[:, j] = mon(Resource_grid[:, j])
            cap = interp1d(
                np.squeeze(np.asarray(res_list[j].copy())),
                np.squeeze(np.asarray(cap_list[j].copy())),
                fill_value="extrapolate",
            )
            ...
            c_a_new[log_index, j] = (
                Resource_grid[log_index, j].copy()
                + labor_inc_grid[log_index, j].copy()
                - grid["m"].T[0]
            )
        ...
        c_a_new = np.reshape(
            c_a_new.copy(), (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
        )
        k_a_new = np.reshape(
            k_a_new.copy(), (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
        )
        m_a_new = np.reshape(
            m_a_new.copy(), (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
        )
    ...
    def PolicyGuess(self, meshes, WW, RR, RBRB, par, mpar):
        ...
        inc["profits"] = 0.0  # lumpsum profits
    ...
    def FactorReturns(self, meshes, grid, par, mpar):
        ...
        """
        ## GHH preferences
        mc = par["mu"] - ((par["beta"] - 1.0) * np.log(par["PI"])) / par["kappa"]
        N = (par["tau"] * par["alpha"] * grid["K"] ** (1.0 - par["alpha"]) * mc) ** (
            1.0 / (1.0 - par["alpha"] + par["gamma"])
        )
        W_fc = par["alpha"] * mc * (grid["K"] / N) ** (1.0 - par["alpha"])

        # Before tax return on capital
        R_fc = (1.0 - par["alpha"]) * mc * (N / grid["K"]) ** par["alpha"] - par[
            "delta"
        ]

        Y = N ** par["alpha"] * grid["K"] ** (1.0 - par["alpha"])
        Profits_fc = (1.0 - mc) * Y - Y * (1.0 / (1.0 - par["mu"])) / par[
            "kappa"
        ] / 2.0 * np.log(par["PI"]) ** 2.0

        NW = par["gamma"] / (1.0 + par["gamma"]) * N / par["H"] * W_fc
    ...
    def StochasticsVariance(self, par, mpar, grid):
        ...
        TauchenResult = Tauchen(par["rhoH"], mpar["nh"] - 1, 1.0, 0.0, mpar["tauchen"])
        ...
        P_H = Transition(
            mpar["nh"] - 1, par["rhoH"], np.sqrt(1 - par["rhoH"] ** 2), boundsH
        )

        # Transitions to enterpreneur state
        P_H = np.concatenate(
            (P_H.copy(), np.tile(mpar["in"], (mpar["nh"] - 1, 1))), axis=1
        )
        lastrow = np.concatenate(
            (np.tile(0.0, (1, mpar["nh"] - 1)), [[1 - mpar["out"]]]), axis=1
        )
    ...
    def SteadyState(self, P_H, grid, meshes, mpar, par):
        ...
        joint_distr = np.ones((mpar["nm"], mpar["nk"], mpar["nh"])) / (
            mpar["nh"] * mpar["nk"] * mpar["nm"]
        )
        ...
        return {
            "c_n_guess": r1_excessK["c_n_guess"],
            "m_n_star": r1_excessK["m_n_star"],
            "c_a_guess": r1_excessK["c_a_guess"],
            "m_a_star": r1_excessK["m_a_star"],
            "cap_a_star": r1_excessK["cap_a_star"],
            "psi_guess": r1_excessK["psi_guess"],
            "joint_distr": r1_excessK["joint_distr"],
            "R_fc": r1_excessK["R_fc"],
            "W_fc": r1_excessK["W_fc"],
            "Profits_fc": r1_excessK["Profits_fc"],
            "Output": r1_excessK["Output"],
            "grid": grid,
        }
    ...
    def ExcessK(
        ...
        pSS_ek_result = self.PoliciesSS(
            c_a_guess, c_n_guess, psi_guess, grid, inc, RR, RBRB, P_H, mpar, par, meshes
        )
        ...
        start_time = time.clock()
        jd_ek_result = self.JDiteration(
            joint_distr, m_n_star, m_a_star, cap_a_star, P_H, par, mpar, grid
        )
        joint_distr = np.reshape(
            jd_ek_result["joint_distr"].copy(),
            (mpar["nm"], mpar["nk"], mpar["nh"]),
            order="F",
        )
        ...
        AggregateCapitalDemand = np.sum(
            grid["k"] * np.sum(np.sum(joint_distr.copy(), axis=0), axis=1)
        )
