...
def Transition(N, rho, sigma_e, bounds):
    ...
    P : np.array    
        transition matrix
    """
    pijfunc = lambda x, bound1, bound2: norm.pdf(x) * (
        norm.cdf((bound2 - rho * x) / sigma_e) - norm.cdf((bound1 - rho * x) / sigma_e)
    )
    ...
    for i in range(0, int(np.floor((N - 1) / 2) + 1)):
        for j in range(0, N):
            ...
            pijvalue, err = sc.integrate.quad(
                pijfunc, bounds[i], bounds[i + 1], args=(bounds[j], bounds[j + 1])
            )
            P[i, j] = pijvalue / (norm.cdf(bounds[i + 1]) - norm.cdf(bounds[i]))

    P[int(np.floor((N - 1) / 2) + 1) : N, :] = P[
        int(np.ceil((N - 1) / 2) - 1) :: -1, ::-1
    ]
    ps = np.sum(P, axis=1)
...
def ExTransitions(S, grid, mpar, par):
    ...
    lastrow = np.concatenate(
        (np.zeros((1, mpar["nh"] - 1)), [[1 - mpar["out"]]]), axis=1
    )
...
def GenWeight(x, xgrid):
    ...
    weight = (x - xgrid[index]) / (
        xgrid[index + 1] - xgrid[index]
    )  # weight xm of higher gridpoint
...
def MakeGrid2(mpar, grid, m_min, m_max):
    ...
    grid["m"] = (
        np.exp(
            np.exp(
                np.linspace(0.0, np.log(np.log(m_max - m_min + 1) + 1), mpar["nm"] - 1)
            )
            - 1
        )
        - 1
    ...
    grid["m"] = np.sort(np.append(grid["m"], 0.0))
...
def MakeGridkm(mpar, grid, k_min, k_max, m_min, m_max):
    ...
    """
    grid["k"] = (
        np.exp(np.linspace(0.0, np.log(k_max - k_min + 1.0), mpar["nk"])) - 1 + k_min
    )  # set up quadruple exponential grid
...
def Tauchen(rho, N, sigma, mue, types):
    ...
    if types in {"importance", "equi", "simple", "simple importance"}:
        ...
        types = "importance"
        print(
            "Warning: TAUCHEN:NoOpt",
            "No valid type set. Importance sampling used instead",
        )
    ...
    if types == "importance":  # Importance sampling
        ...
        for i in range(N):
            ...
            for j in range(1, N - 1):
                P[i, j] = norm.cdf(
                    (grid[j] + step / 2 - rho * grid[i]) / sigma_e
                ) - norm.cdf((grid[j] - step / 2 - rho * grid[i]) / sigma_e)

    elif types == "simple importance":  # use simple transition probabilities

        grid_probs = np.linspace(0.0, 1.0, N + 1)
        ...
        for i in range(int(np.floor((N - 1) / 2)) + 1):
            ...
            for j in range(int(np.floor((N - 1) / 2)) + 1):
                P[i, j] = norm.cdf(
                    (bounds[j + 1] - rho * grid[i]) / sigma_e
                ) - norm.cdf((bounds[j] - rho * grid[i]) / sigma_e)

        P[int(np.floor((N - 1) / 2)) + 1 :, :] = P[
            int(np.ceil((N - 1) / 2)) - 1 :: -1, ::-1
        ].copy()
...
def Fastroot(xgrid, fx):
    ...
    idx_max = (
        fx[-1, :] < 0
    )  # Corner solutions right (if no solution x* to f(x)=0 exists)
    index = np.squeeze(
        np.asarray((np.where((~idx_min) & (~idx_max))))
    )  # interior solutions (if solution x* to f(x)=0 exists)

    # Find index of two gridpoints where sign of fx changes from positive to negative,
    idx[index] = np.asmatrix(
        np.argmax(np.diff(np.sign(fx[:, index]), axis=0), axis=0)
    ).T

    aux_index = np.asmatrix(
        np.arange(0, np.size(fx) // np.size(xgrid), 1) * np.size(xgrid)
    ).T  # aux for linear indexes
