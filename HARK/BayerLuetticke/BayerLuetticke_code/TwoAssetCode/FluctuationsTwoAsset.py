    ...
    def StateReduc(self):
        ...
        Xss = np.asmatrix(
            np.concatenate(
                (
                    np.sum(
                        np.sum(self.joint_distr.copy(), axis=1), axis=1
                    ),  # marginal distribution liquid asset
                    np.transpose(
                        np.sum(np.sum(self.joint_distr.copy(), axis=0), axis=1)
                    ),  # marginal distribution illiquid asset
                    np.sum(
                        np.sum(self.joint_distr.copy(), axis=1), axis=0
                    ),  # marginal distribution productivity
                    [np.log(self.par["RB"])],
                    [0.0],
                )
            )
...
if __name__ == "__main__":
    ...
    EX3SS = pickle.load(
        open("HARK/BayerLuetticke/BayerLuetticke_code/TwoAssetCode/EX3SS_20.p", "rb")
    )
