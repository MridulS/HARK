...
class StateReduc_Dct:
    def __init__(
        ...
        self.P_H = (
            P_H
        )  # Transition matrix for macro states (not including distribution)

    def StateReduc(self):
        # Inverse of CRRA on x for utility and marginal utility
        invutil = lambda x: ((1 - self.par["xi"]) * x) ** (1.0 / (1 - self.par["xi"]))
        invmutil = lambda x: (1.0 / x) ** (1.0 / self.par["xi"])
        ...
        Xss = np.asmatrix(
            np.concatenate(
                (
                    np.sum(np.sum(self.joint_distr.copy(), axis=1), axis=1),
                    np.transpose(
                        np.sum(np.sum(self.joint_distr.copy(), axis=0), axis=1)
                    ),  # marg dist k
                    np.sum(
                        np.sum(self.joint_distr.copy(), axis=1), axis=0
                    ),  # marg dist pty (\approx income)
                    [np.log(self.par["RB"])],
                    [0.0],
                )
            )
        ).T  # Given the constant interest rate on bonds/liquid assets

        # Y="controls" (according to this literature's odd terminology)
        Yss = np.asmatrix(
            np.concatenate(
                (
                    invmutil(self.mutil_c.copy().flatten(order="F")),
                    ...
                    [np.log(self.par["N"])],
                    [np.log(self.targets["T"])],
                    [np.log(self.grid["K"])],
                    [np.log(self.targets["B"])],
                )
            )
        ).T  # Government debt
        ...
        for j in range(
            self.mpar["nm"] - 1
        ):  # np.squeeze reduces one-dimensional matrix to vector
            Gamma_state[0 : self.mpar["nm"], j] = -np.squeeze(Xss[0 : self.mpar["nm"]])
            Gamma_state[j, j] = 1.0 - Xss[j]  #
            Gamma_state[j, j] = Gamma_state[j, j] - np.sum(
                Gamma_state[0 : self.mpar["nm"], j]
            )
        bb = self.mpar[
            "nm"
        ]  # Mnemonic bb='bottom base'; because bb shorter to type than self.mpar['nm'] everywhere

        # WangTao: Replace magic numbers or obscure variables with more self-explanatory names
        for j in range(self.mpar["nk"] - 1):
            Gamma_state[
                bb + np.arange(0, self.mpar["nk"], 1), bb + j - 1
            ] = -np.squeeze(Xss[bb + np.arange(0, self.mpar["nk"], 1)])
            Gamma_state[bb + j, bb - 1 + j] = 1.0 - Xss[bb + j]
            Gamma_state[bb + j, bb - 1 + j] = Gamma_state[bb + j, bb - 1 + j] - np.sum(
                Gamma_state[bb + np.arange(0, self.mpar["nk"]), bb - 1 + j]
            )
        bb = self.mpar["nm"] + self.mpar["nk"]

        for j in range(
            self.mpar["nh"] - 2
        ):  # Question: -2 because max human wealth not perturbed?
            Gamma_state[
                bb + np.arange(0, self.mpar["nh"] - 1, 1), bb + j - 2
            ] = -np.squeeze(Xss[bb + np.arange(0, self.mpar["nh"] - 1, 1)])
            Gamma_state[bb + j, bb - 2 + j] = 1.0 - Xss[bb + j]
            Gamma_state[bb + j, bb - 2 + j] = Gamma_state[bb + j, bb - 2 + j] - np.sum(
                Gamma_state[bb + np.arange(0, self.mpar["nh"] - 1, 1), bb - 2 + j]
            )

        # Number of other state variables not including the gridded -- here, just the interest rate
        self.mpar["os"] = len(Xss) - (
            self.mpar["nm"] + self.mpar["nk"] + self.mpar["nh"]
        )
        ...
        indexMUdct = self.do_dct(
            invmutil(self.mutil_c.copy().flatten(order="F")), self.mpar, accuracy
        )
        ...
        self.mpar["numcontrols"] = np.int64(
            len(indexMUdct) + len(indexVKdct) + self.mpar["oc"]
        )
        ...
        return {
            ...
            "P_H": self.P_H,
            "joint_distr": self.joint_distr,
            "Output": self.Output,
            "indexMUdct": indexMUdct,
            "indexVKdct": indexVKdct,
            "State": State,
            "State_m": State_m,
            "Contr": Contr,
            "Contr_m": Contr_m,
...
def Fsys(
    ...
    mutil = lambda x: 1.0 / np.power(x, par["xi"])
    #    invmutil = lambda x : (1./x)**(1./par['xi'])
    invmutil = lambda x: np.power(1.0 / x, 1.0 / par["xi"])
    ...
    Vkind = len(indexMUdct) + np.array(
        range(len(indexVKdct))
    )  # capital stacks after consumption
    ...
    marginal_hind = range(
        mpar["nm"] + mpar["nk"] - 2, mpar["nm"] + mpar["nk"] + mpar["nh"] - 4
    )
    ...
    Control[-mpar["oc"] :] = (
        ControlSS[-mpar["oc"] :].copy() + Control_sparse[-mpar["oc"] :, :].copy()
    )
    Controlminus[-mpar["oc"] :] = (
        ControlSS[-mpar["oc"] :].copy() + Controlminus_sparse[-mpar["oc"] :, :].copy()
    )
    ...
    mutil_c = mutil(
        mutil_c_dev.copy().flatten(order="F")
    ...
    LHS[marginal_hind] = Distribution[
        mpar["nm"] + mpar["nk"] : mpar["nm"] + mpar["nk"] + mpar["nh"] - 2
    ]
    ...
    marginal_kminus = np.transpose(
        Distributionminus[mpar["nm"] : mpar["nm"] + mpar["nk"]].copy()
    )
    marginal_hminus = np.transpose(
        Distributionminus[
            mpar["nm"] + mpar["nk"] : mpar["nm"] + mpar["nk"] + mpar["nh"]
        ].copy()
    )
    ...
    RHS[
        nx + Bind
    ] = Lminus  # LHS[nx+Bind] was Bminus because liquid assets are gov debt
    ...
    cm, ck, ch = np.meshgrid(
        np.asarray(np.cumsum(marginal_mminus)),
        np.asarray(np.cumsum(marginal_kminus)),
        np.asarray(np.cumsum(marginal_hminus)),
        indexing="ij",
    )
    ...
    Copula_bounds = griddata(
        ...
        method="nearest",
    )  # Nearest point in the grid
    # Replacing NAN's with the nearest ones
    Copula_aux[np.isnan(Copula_aux.copy())] = Copula_bounds[
        np.isnan(Copula_aux.copy())
    ].copy()

    # Reshape it into a proper CDF
    cumdist[1:, 1:, 1:] = np.reshape(
        Copula_aux, (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
    )
    ...
    mc = (
        par["mu"]
        - (par["beta"] * np.log(PI) * Y / Yminus - np.log(PIminus)) / par["kappa"]
    )

    # Aggregate hours worked
    RHS[nx + Nind] = np.power(
        par["tau"] * TFP * par["alpha"] * np.power(Kminus, (1.0 - par["alpha"])) * mc,
        1.0 / (1.0 - par["alpha"] + par["gamma"]),
    )
    # Aggregate output (Cobb-Douglas)
    RHS[nx + Yind] = (
        TFP * np.power(Nminus, par["alpha"]) * np.power(Kminus, 1.0 - par["alpha"])
    )
    ## Prices that are not a part of control vector
    # Wage Rate depends on the production function and the markup mc
    RHS[nx + Wind] = (
        TFP * par["alpha"] * mc * np.power((Kminus / Nminus), 1.0 - par["alpha"])
    )
    # Return on Capital
    RHS[nx + Rind] = (
        TFP * (1.0 - par["alpha"]) * mc * np.power((Nminus / Kminus), par["alpha"])
        - par["delta"]
    )
    ...
    RHS[nx + Profitind] = (
        (1.0 - mc) * Yminus
        - Yminus * (1.0 / (1.0 - par["mu"])) / par["kappa"] / 2.0 * np.log(PIminus) ** 2
        + 1.0 / 2.0 * par["phi"] * ((K - Kminus) ** 2) / Kminus
    )

    ## Wages net of leisure services
    ## Uses GHH preferences
    WW = (
        par["gamma"] / (1.0 + par["gamma"]) * (Nminus / Hminus) * Wminus
    ).item() * np.ones((mpar["nm"], mpar["nk"], mpar["nh"]))
    WW[:, :, -1] = (
        Profitminus.item() * par["profitshare"] * np.ones((mpar["nm"], mpar["nk"]))
    )
    ...
    inc["money"] = meshes["m"].copy() * (
        RBminus.item() / PIminus.item()
        + (meshes["m"] < 0) * par["borrwedge"] / PIminus.item()
    )
    ...
    EVk = np.reshape(
        np.asarray(
            np.reshape(Vk.copy(), (mpar["nm"] * mpar["nk"], mpar["nh"]), order="F").dot(
                P.copy().T
            )
        ),
    ...
    EVm = np.reshape(
        np.asarray(
            np.reshape(
                np.multiply(
                    RBaux.flatten(order="F").T.copy(), mutil_c.flatten(order="F").copy()
                ),
                (mpar["nm"] * mpar["nk"], mpar["nh"]),
                order="F",
            ).dot(np.transpose(P.copy()))
        ),
        (mpar["nm"], mpar["nk"], mpar["nh"]),
        order="F",
    )
    # Update policies using endogenous gridpoints method for out of steady state stuff
    result_EGM_policyupdate = EGM_policyupdate(
        EVm,
        EVk,
        Qminus.item(),
        PIminus.item(),
    ...
    EVk = np.reshape(Vk, (mpar["nm"] * mpar["nk"], mpar["nh"]), order="F").dot(
        P.copy().T
    )
    ...
    Vpoints = np.concatenate(
        (
            [meshaux["m"].flatten(order="F")],
            [meshaux["k"].flatten(order="F")],
            [meshaux["h"].flatten(order="F")],
        ),
        axis=0,
    ).T
    ...
    Vk_next = griddata(
        Vpoints,
        np.asarray(EVk).flatten(order="F").copy(),
        (
            m_n_star.copy().flatten(order="F"),
            meshaux["k"].copy().flatten(order="F"),
            meshaux["h"].copy().flatten(order="F"),
        ),
        method="linear",
    )
    Vk_next_bounds = griddata(
        Vpoints,
        np.asarray(EVk).flatten(order="F").copy(),
        (
            m_n_star.copy().flatten(order="F"),
    ...
    Vk_aux = (
        par["nu"] * (Rminus.item() + Qminus.item()) * mutil_c_a
        + (1 - par["nu"]) * Rminus.item() * mutil_c_n
        + par["beta"]
        * (1 - par["nu"])
        * np.reshape(Vk_next, (mpar["nm"], mpar["nk"], mpar["nh"]), order="F")
    )

    aux = invmutil(Vk_aux.copy().flatten(order="F")) - np.squeeze(
        np.asarray(ControlSS[np.array(range(NN)) + NN])
    )
    ...
    idk_n = np.reshape(
        np.tile(
            np.outer(np.ones((mpar["nm"])), np.array(range(mpar["nk"]))),
            (1, 1, mpar["nh"]),
        ),
        (mpar["nm"], mpar["nk"], mpar["nh"]),
        order="F",
    ...
    idh = np.kron(
        np.array(range(mpar["nh"])), np.ones((1, mpar["nm"] * mpar["nk"] * mpar["nh"]))
    )
    ...
    index11 = np.ravel_multi_index(
        [idm_a.flatten(order="F"), idk_a.flatten(order="F"), idh.flatten(order="F")],
        (mpar["nm"], mpar["nk"], mpar["nh"]),
        order="F",
    )
    ...
    indexn1 = np.ravel_multi_index(
        [idm_n.flatten(order="F"), idk_n.flatten(order="F"), idh.flatten(order="F")],
        (mpar["nm"], mpar["nk"], mpar["nh"]),
        order="F",
    )
    indexn2 = np.ravel_multi_index(
        [
    ...
    for hh in range(mpar["nh"]):
        ...
        weight11[:, :, hh] = np.outer(
            weight11_aux.flatten(order="F").copy(), P[hh, :].copy()
        )
        weight12[:, :, hh] = np.outer(
            weight12_aux.flatten(order="F").copy(), P[hh, :].copy()
        )
        weight21[:, :, hh] = np.outer(
            weight21_aux.flatten(order="F").copy(), P[hh, :].copy()
        )
        weight22[:, :, hh] = np.outer(
            weight22_aux.flatten(order="F").copy(), P[hh, :].copy()
        )

        weightn1[:, :, hh] = np.outer(
            weightn1_aux.flatten(order="F").copy(), P[hh, :].copy()
        )
        weightn2[:, :, hh] = np.outer(
            weightn2_aux.flatten(order="F").copy(), P[hh, :].copy()
        )
    ...
    H_n = sp.coo_matrix(
        (
            np.hstack((weightn1.flatten(order="F"), weightn2.flatten(order="F"))),
            (
                np.squeeze(rowindex),
                np.hstack(
                    (np.squeeze(np.asarray(indexn1)), np.squeeze(np.asarray(indexn2)))
                ),
            ),
        ),
        shape=(
    ...
    JD_new = (
        JDminus.flatten(order="F").copy().dot(H.todense())
    )  # dot product because sparse
    JD_new = np.reshape(
        np.asarray(JD_new.copy()), (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
    )
    ...
    RHS[RBind] = (
        np.log(par["RB"])
        + par["rho_R"] * np.log(RBminus / par["RB"])
        + np.log(PIminus / par["PI"]) * ((1.0 - par["rho_R"]) * par["theta_pi"])
        + EPS_TAYLOR
    )
    ...
    if par["tau"] < 1:  # 1 minus tax rate
        ...
        RHS[nx + PIind] = (
            par["rho_B"] * np.log(Bminus / targets["B"])
        ...
        RHS[nx + Qind] = (par["phi"] * (K / Kminus - 1) + 1) - par[
            "ABS"
        ]  # ABS is loan to value max
...
def EGM_policyupdate(EVm, EVk, Qminus, PIminus, RBminus, inc, meshes, grid, par, mpar):
    ...
    ## EGM step 1
    EMU = par["beta"] * np.reshape(
        EVm.copy(), (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
    )
    c_new = 1.0 / np.power(EMU, (1.0 / par["xi"]))
    ...
    m_star_n = (
        c_new.copy() + meshes["m"].copy() - inc["labor"].copy() - inc["rent"].copy()
    )
    m_star_n = m_star_n.copy() / (
        RBminus / PIminus + (m_star_n.copy() < 0) * par["borrwedge"] / PIminus
    )

    # Identify binding constraints
    binding_constraints = meshes["m"].copy() < np.tile(
    ...
    if (
        np.sum(
            np.abs(np.diff(np.sign(np.diff(m_star_n.copy(), axis=0)), axis=0)), axis=1
        ).max()
        != 0.0
    ):
    ...
    for hh in range(mpar["nk"] * mpar["nh"]):
        ...
        Savings = interp1d(
            np.squeeze(np.asarray(m_star_n[:, hh].copy())),
            grid["m"].copy(),
        ...
        Consumption = interp1d(
            np.squeeze(np.asarray(m_star_n[:, hh].copy())),
            np.squeeze(np.asarray(c_n_aux[:, hh].copy())),
            fill_value="extrapolate",
        )
    ...
    c_n_star[binding_constraints] = np.squeeze(
        np.asarray(Resource[binding_constraints].copy() - grid["m"][0])
    )
    ...
    term1 = par["beta"] * np.reshape(
        EVk, (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
    )

    E_return_diff = term1 / Qminus - EMU

    # Check quasi-monotonicity of E_return_diff
    if np.sum(np.abs(np.diff(np.sign(E_return_diff), axis=0)), axis=0).max() > 2.0:
    ...
    s = (m_a_aux.copy() - grid["m"][idx]) / step[
        idx
    ]  # Distance of optimal policy to next grid point

    aux_index = (
        np.array(range(0, (mpar["nk"] * mpar["nh"]))) * mpar["nm"]
    )  # aux for linear indexes
    aux3 = EMU.flatten(order="F").copy()[
        idx.flatten(order="F").copy() + aux_index.flatten(order="F").copy()
    ]  # calculate linear indexes

    # Interpolate EMU(m',k',s'*h',M',K') over m*_n(k'), m-dim is dropped
    EMU_star = aux3 + s.flatten(order="F") * (
        EMU.flatten(order="F").copy()[
            idx.flatten(order="F").copy() + aux_index.flatten(order="F").copy() + 1
        ]
        - aux3
    ...
    for j in range(mpar["nh"]):
        ...
        cons_list_1.append(
            np.vstack((np.asmatrix(cons_list[j]).T, np.asmatrix(c_a_aux[:, j]).T))
        )
        res_list_1.append(
            np.vstack((np.asmatrix(res_list[j]).T, np.asmatrix(Resource[:, j]).T))
        )
        mon_list_1.append(
            np.vstack((np.asmatrix(mon_list[j]).T, np.asmatrix(m_a_aux[:, j]).T))
        )
        cap_list_1.append(
            np.vstack((np.asmatrix(cap_list[j].copy()).T, np.asmatrix(grid["k"]).T))
        )
    ...
    labor_inc_grid = np.reshape(
        inc["labor"], (mpar["nm"] * mpar["nk"], mpar["nh"]), order="F"
    )
    ...
    for j in range(mpar["nh"]):
        ...
        if np.sum(np.abs(np.diff(np.sign(np.diff(res_list[j])))), axis=0).max() != 0.0:
            print("warning(non monotone resource list encountered)")
        cons = interp1d(
            np.squeeze(np.asarray(res_list_1[j].copy())),
            np.squeeze(np.asarray(cons_list_1[j].copy())),
            fill_value="extrapolate",
        )
        c_a_star[:, j] = cons(Resource_grid[:, j].copy())
        mon = interp1d(
            np.squeeze(np.asarray(res_list_1[j].copy())),
            np.squeeze(np.asarray(mon_list_1[j].copy())),
            fill_value="extrapolate",
        )
        m_a_star[:, j] = mon(Resource_grid[:, j].copy())
        cap = interp1d(
            np.squeeze(np.asarray(res_list_1[j].copy())),
            np.squeeze(np.asarray(cap_list_1[j].copy())),
            fill_value="extrapolate",
        )
        ...
        c_a_star[log_index, j] = (
            Resource_grid[log_index, j].copy()
            + labor_inc_grid[log_index, j].copy()
            - grid["m"][0]
        )
        m_a_star[log_index, j] = grid["m"][0]
        k_a_star[log_index, j] = 0.0

    c_a_star = np.reshape(
        c_a_star.copy(), (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
    )
    k_a_star = np.reshape(
        k_a_star.copy(), (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
    )
    m_a_star = np.reshape(
        m_a_star.copy(), (mpar["nm"], mpar["nk"], mpar["nh"]), order="F"
    )
...
def plot_IRF(mpar, par, gx, hx, joint_distr, Gamma_state, grid, targets, Output):
    ...
    IRF_distr = (
        Gamma_state
        * IRF_state_sparse[: mpar["numstates"] - mpar["os"], : mpar["maxlag"]]
    )
    ...
    IRF_H = (
        ...
        * grid["h"][:-1]
        * IRF_distr[
            mpar["nm"] + mpar["nk"] : mpar["nm"] + mpar["nk"] + mpar["nh"] - 1, 1:
        ]
        / par["H"]
    )
    K = np.asarray(
        grid["k"] * IRF_distr[mpar["nm"] : mpar["nm"] + mpar["nk"], :] + grid["K"]
    ).T
    ...
    IRF_K = (
        100
        * grid["k"]
        * IRF_distr[mpar["nm"] : mpar["nm"] + mpar["nk"], 1:]
        / grid["K"]
    )
    IRF_M = (
        100
        * grid["m"]
        * IRF_distr[: mpar["nm"], 1:]
        / (targets["B"] + par["ABS"] * grid["K"])
    )
    K = K.copy().T
    M = (
        grid["m"] * IRF_distr[: mpar["nm"], :]
        + targets["B"]
        - par["ABS"] * (K - grid["K"])
    )
    ...
    IRF_LP = (
        ...
        * 100
        * (
            ((Q[:, 1:] + R[:, 1:]) / Q[:, :-1] - RB[:, :-1] / PI[:, 1:])
            - ((1 + par["R"] / par["Q"]) - par["RB"])
        )
    )

    f_Y = plt.figure(1)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_Y)), label="IRF_Y"
    )
    ...
    f_C = plt.figure(2)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_C)), label="IRF_C"
    )
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.legend(handles=[line1])
    plt.xlabel("Quarter")
    ...
    f_I = plt.figure(3)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_I)), label="IRF_I"
    )
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.legend(handles=[line1])
    plt.xlabel("Quarter")
    ...
    f_G = plt.figure(4)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_G)), label="IRF_G"
    )
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    # plt.ylim((-1, 1))
    plt.legend(handles=[line1])
    ...
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(Deficit)), label="IRF_Deficit"
    )
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.legend(handles=[line1])
    plt.xlabel("Quarter")
    ...
    f_K = plt.figure(6)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_K)), label="IRF_K"
    )
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.legend(handles=[line1])
    plt.xlabel("Quarter")
    ...
    f_M = plt.figure(7)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_M)), label="IRF_M"
    )
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.legend(handles=[line1])
    plt.xlabel("Quarter")
    ...
    f_H = plt.figure(8)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_H)), label="IRF_H"
    )
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.legend(handles=[line1])
    plt.xlabel("Quarter")
    ...
    f_S = plt.figure(10)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_S)), label="IRF_S"
    )
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.legend(handles=[line1])
    plt.xlabel("Quarter")
    ...
    line2, = plt.plot(
        range(1, mpar["maxlag"]),
        np.squeeze(np.asarray(IRF_RBREAL)),
        label="real",
        color="blue",
    )
    ...
    f_RB = plt.figure(12)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_RB)), label="IRF_RB"
    )
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.legend(handles=[line1])
    plt.xlabel("Quarter")
    ...
    f_PI = plt.figure(13)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_PI)), label="IRF_PI"
    )
    plt.legend(handles=[line1])
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.xlabel("Quarter")
    ...
    f_Q = plt.figure(14)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_Q)), label="IRF_Q"
    )
    plt.legend(handles=[line1])
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.xlabel("Quarter")
    ...
    f_D = plt.figure(15)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_D)), label="IRF_D"
    )
    plt.legend(handles=[line1])
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.xlabel("Quarter")
    ...
    f_LP = plt.figure(16)
    line1, = plt.plot(
        range(1, mpar["maxlag"] - 1), np.squeeze(np.asarray(IRF_LP)), label="IRF_LP"
    )
    plt.legend(handles=[line1])
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.xlabel("Quarter")
    ...
    f_N = plt.figure(17)
    line1, = plt.plot(
        range(1, mpar["maxlag"]), np.squeeze(np.asarray(IRF_N)), label="IRF_N"
    )
    plt.legend(handles=[line1])
    plt.plot(range(0, mpar["maxlag"]), np.zeros((mpar["maxlag"])), "k--")
    plt.xlabel("Quarter")
...
def SGU_solver(
    ...
    for bl in range(0, blocks):
        ...
        for Xct in range_:
            ...
            DF3[:, Xct - bl * packagesize] = (
                Fx["Difference"] - Fb
            ) / h  # Deriv divided by dev
    ...
    for bl in range(0, blocks):
        range_ = range(
            bl * packagesize, min(packagesize * (bl + 1), mpar["numcontrols"])
        )
    ...
    abst = abs(np.diag(t)) * (abs(np.diag(t)) != 0.0) + (
        abs(np.diag(t)) == 0.0
    ) * 10 ** (-11)
    ...
    s_ord, t_ord, __, __, __, Z_ord = linalg.ordqz(
        np.hstack((F1, F2)), -np.hstack((F3, F4)), sort="ouc", output="complex"
    )
    ...
    if nk > mpar["numstates"]:
        if mpar["overrideEigen"]:
            ...
            s_ord, t_ord, __, __, __, Z_ord = linalg.ordqz(
                np.hstack((F1, F2)),
                -np.hstack((F3, F4)),
                sort=sortOverridEigen,
                output="complex",
            )

        else:
            print(
                "No Local Equilibrium Exists, last eigenvalue: ",
                str(ll[-1 - mpar["numstates"]]),
            )
