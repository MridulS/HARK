...
class BayerLuettickeAgent(AgentType):
    ...
    def simBirth(self, which_agents):
        ...
        mgrid = self.mgrid
        col_indicies = np.repeat(
            [range(joint_distr.shape[1])], joint_distr.shape[0], 0
        ).flatten()
        row_indicies = np.transpose(
            np.repeat([range(joint_distr.shape[0])], joint_distr.shape[1], 0)
        ).flatten()
        draws = drawDiscrete(
            N,
    ...
    def getShocks(self):
        ...
        for j in range(self.incStateTransition.shape[0]):
            ...
            incStateNow[these] = np.searchsorted(
    ...
    def getPostStates(self):
        ...
        PI = par["PI"]
        RR = (RB + (bPrev.copy() < 0.0) * borrwedge) / PI
        W = par["W"]
        N = par["N"]
        H = par["H"]
        Profits = par["PROFITS"]
    ...
    def getEconomyData(self, Economy):
        ...
        for j in range(self.numIncStates):
            SSConsumptionFunc_j = interp1d(
                self.m_policy[:, j], self.c_policy[:, j], fill_value="extrapolate"
            )
...
if __name__ == "__main__":
    ...
    BayerLuettickeExampleEconomy = BayerLuettickeEconomy(
        FluctuationsOneAssetIOU, agents=[BayerLuettickeExampleAgent]
    )
    # Solve the market
    ...
    if simulate:
        ...
        for t in range(BayerLuettickeExampleAgent.T_sim):
            meanbNow0[t] = np.mean(
                BayerLuettickeExampleAgent.bNow_hist[t, :][
                    BayerLuettickeExampleAgent.incStateNow_hist[t, :] == 0
                ]
            )
            meanbNow1[t] = np.mean(
                BayerLuettickeExampleAgent.bNow_hist[t, :][
                    BayerLuettickeExampleAgent.incStateNow_hist[t, :] == 1
                ]
            )
            meanbNow2[t] = np.mean(
                BayerLuettickeExampleAgent.bNow_hist[t, :][
                    BayerLuettickeExampleAgent.incStateNow_hist[t, :] == 2
                ]
            )
            meanbNow3[t] = np.mean(
                BayerLuettickeExampleAgent.bNow_hist[t, :][
                    BayerLuettickeExampleAgent.incStateNow_hist[t, :] == 3
                ]
            )
