...
print(
    ...
    + "\n gridpoints have dimensionality of "
    + str(EX3SS["Copula"]["grid"].shape)
    + "\n where the first element is total number of gridpoints"
    + "\n and the second element is number of idiosyncratic state variables"
    + "\n whose values also are of dimension of "
    + str(EX3SS["Copula"]["value"].shape[0])
...
class StateReduc_Dct:
    ...
    def StateReduc(self):
        ...
        ).T  # Given the constant interest rate
        ...
        Yss = np.asmatrix(
            np.concatenate(
                (
                    ...
                    [
                        np.log(self.par["Q"])
                    ],  # Question: Price of the illiquid asset, right?
        ...
        bb = self.mpar[
            "nm"
        ]  # Question: bb='bottom base'? because bb shorter to type than self.mpar['nm'] everywhere
    ...
    def do_dct(self, obj, mpar, level):
        ...
        X1 = sf.dct(
            obj, norm="ortho", axis=0
        )  # dct is operated along three dimensions axis=0/1/2
...
EX3SR = StateReduc_Dct(
    **EX3SS
)  # Takes StE result as input and get ready to invoke state reduction operation
...
print(
    "The dimension of the policy functions is reduced to "
    + str(SR["indexMUdct"].shape[0])
    + " from "
    + str(EX3SS["mpar"]["nm"] * EX3SS["mpar"]["nk"] * EX3SS["mpar"]["nh"])
)
print(
    "The dimension of the marginal value functions is reduced to "
    + str(SR["indexVKdct"].shape[0])
    + " from "
    + str(EX3SS["Vk"].shape)
)
print(
    "The total number of control variables is "
    + str(SR["Contr"].shape[0])
    + "="
    + str(SR["indexMUdct"].shape[0])
    + "+"
    + str(SR["indexVKdct"].shape[0])
    + "+ # of other macro controls"
)
print("\n")
print(
    "The copula represents the joint distribution with a vector of size "
    + str(SR["Gamma_state"].shape)
)
print(
...
print(
    "The total number of state variables is "
    + str(SR["State"].shape[0])
    + "="
    + str(SR["Gamma_state"].shape[1])
    + "+ the number of macro states (like the interest rate)"
)
...
xi = EX3SS["par"]["xi"]
invmutil = lambda x: (1.0 / x) ** (1.0 / xi)
...
fig.suptitle(
    "Consumption of non-adjusters at grid points of m and k(for different h)",
    fontsize=(13),
)
for hgrid_id in range(EX3SS["mpar"]["nh"]):
    ...
    mmax = mgrid[
        (np.abs(marginal_m.cumsum() - mass_pct * marginal_m.cumsum().max())).argmin()
    ]
    kmax = kgrid[
        (np.abs(marginal_k.cumsum() - mass_pct * marginal_k.cumsum().max())).argmin()
    ]
...
fig = plt.figure(figsize=(14, 14))
fig.suptitle(
    "Consumption of adjusters at grid points of m and k(for different h)", fontsize=(13)
)
for hgrid_id in range(EX3SS["mpar"]["nh"]):
    ## prepare the reduced grids
    hgrid_fix = hgrid_id
...
fig = plt.figure(figsize=(14, 14))
fig.suptitle(
    "Consumption of adjusters/non-adjusters at grid points of m and k(for different h)",
    fontsize=(13),
)
for hgrid_id in range(EX3SS["mpar"]["nh"]):
    ## prepare the reduced grids
    ...
    ax.plot_surface(mmgrid, kkgrid, c_a_rdc, label="StE(after dct):adjuster")
    ax.set_xlabel("m", fontsize=13)
    ax.set_ylabel("k", fontsize=13)
    ax.set_zlabel(r"$c_a(m,k)$", fontsize=13)
    ax.set_title(r"$h({})$".format(hgrid_fix))
...
fig = plt.figure(figsize=(14, 14))
fig.suptitle("Joint distribution of m and k(for different h)", fontsize=(13))
for hgrid_id in range(EX3SS["mpar"]["nh"]):
    ## plots
    ax = fig.add_subplot(2, 2, hgrid_id + 1, projection="3d")
    ax.plot_surface(
...
fig = plt.figure(figsize=(14, 14))
fig.suptitle("Copula of m and k(for different h)", fontsize=(13))
for hgrid_id in range(EX3SS["mpar"]["nh"]):
    ## plots
    ax = fig.add_subplot(2, 2, hgrid_id + 1, projection="3d")
    ax.plot_surface(
