...
def makeStickyEdataFile(
    ...
    if hasattr(Economy, "agents"):  # If this is a heterogeneous agent specification...
        if len(Economy.agents) > 1:
            pLvlAll_hist = np.concatenate(
                [this_type.pLvlTrue_hist for this_type in Economy.agents], axis=1
            )
            aLvlAll_hist = np.concatenate(
                [this_type.aLvlNow_hist for this_type in Economy.agents], axis=1
            )
            cLvlAll_hist = np.concatenate(
                [this_type.cLvlNow_hist for this_type in Economy.agents], axis=1
            )
            yLvlAll_hist = np.concatenate(
                [this_type.yLvlNow_hist for this_type in Economy.agents], axis=1
            )
        ...
        if (
            ...
            not_newborns = (
                np.concatenate(
                    [
                        this_type.t_age_hist[
                            (ignore_periods + 1) : (
                                ignore_periods + micro_stat_periods
                            ),
                            :,
                        ]
            ...
            Logc = np.log(
                cLvlAll_hist[ignore_periods : (ignore_periods + micro_stat_periods), :]
            )
            DeltaLogc = (Logc[1:] - Logc[0:-1]).flatten()
            DeltaLogc_trimmed = DeltaLogc[not_newborns]
            Loga = np.log(
                aLvlAll_hist[ignore_periods : (ignore_periods + micro_stat_periods), :]
            )
            DeltaLoga = (Loga[1:] - Loga[0:-1]).flatten()
            DeltaLoga_trimmed = DeltaLoga[not_newborns]
            Logp = np.log(
                pLvlAll_hist[ignore_periods : (ignore_periods + micro_stat_periods), :]
            )
            DeltaLogp = (Logp[1:] - Logp[0:-1]).flatten()
            DeltaLogp_trimmed = DeltaLogp[not_newborns]
            Logy = np.log(
                yLvlAll_hist[ignore_periods : (ignore_periods + micro_stat_periods), :]
            )
            Logy_trimmed = Logy
            Logy_trimmed[np.isinf(Logy)] = np.nan
            birth_events = np.concatenate(
                [this_type.t_age_hist == 1 for this_type in Economy.agents], axis=1
            )
            vBirth = calcValueAtBirth(
                cLvlAll_hist[ignore_periods:, :],
                birth_events[ignore_periods:, :],
                PlvlAgg_hist[ignore_periods:],
                Economy.MrkvNow_hist[ignore_periods:],
                Economy.agents[0].DiscFac,
    ...
    if hasattr(Economy, "MrkvNow"):
        ...
        if ~hasattr(Economy, "Rfree") and hasattr(
            Economy, "agents"
        ):  # If this is a markov DSGE specification...
            R = ExpectedR_hist[(ignore_periods + 1) :]
    Delta8LogC = (np.log(ClvlAgg_hist[8:]) - np.log(ClvlAgg_hist[:-8]))[
        (ignore_periods - 7) :
    ]
    Delta8LogY = (np.log(YlvlAgg_hist[8:]) - np.log(YlvlAgg_hist[:-8]))[
        (ignore_periods - 7) :
    ]
    ...
    LogC_long_me = LogC_long + sigma_meas_err * np.random.normal(
        0.0, 1.0, LogC_long.size
    )
    Delta8LogC_me = (LogC_long_me[8:] - LogC_long_me[:-8])[(ignore_periods - 7) :]

    # Make summary statistics for the results file
    csv_output_string = (
        str(np.mean(AnrmAgg_hist[ignore_periods:]))
        + ","
        + str(np.mean(CnrmAgg_hist[ignore_periods:]))
        + ","
        + str(np.std(np.log(AnrmAgg_hist[ignore_periods:])))
        + ","
        + str(np.std(DeltaLogC))
        + ","
        + str(np.std(DeltaLogY))
        + ","
        + str(np.std(DeltaLogA))
    )
    if (
        hasattr(Economy, "agents") and calc_micro_stats
    ):  # This block only runs for heterogeneous agents specifications
        csv_output_string += (
            ","
            + str(np.mean(np.std(Loga, axis=1)))
            + ","
            + str(np.mean(np.std(Logc, axis=1)))
            + ","
            + str(np.mean(np.std(Logp, axis=1)))
            + ","
            + str(np.mean(np.nanstd(Logy_trimmed, axis=1)))
            + ","
            + str(np.std(DeltaLoga_trimmed))
            + ","
            + str(np.std(DeltaLogc_trimmed))
            + ","
            + str(np.std(DeltaLogp_trimmed))
        )
    ...
    if filename is not None:
        ...
        if save_data:
            ...
            if hasattr(Economy, "MrkvNow") & ~hasattr(Economy, "Rfree") and hasattr(
                Economy, "agents"
            ):
...
def runStickyEregressions(infile_name, interval_size, meas_err, sticky, all_specs):
    ...
    CoeffsArray = np.zeros(
        (N, 7)
    )  # Order: DeltaLogC_OLS, DeltaLogC_IV, DeltaLogY_IV, A_OLS, DeltaLogC_HR, DeltaLogY_HR, A_HR
    ...
    for n in range(N):
        ...
        mod_IV = smsrg.IV2SLS(
            DeltaLogC_n[4:], sm.add_constant(DeltaLogC_n[3:-1]), instruments
        )
        res_IV = mod_IV.fit()
        ...
        res_2ndStage = mod_2ndStage.fit()
        mod_IV = smsrg.IV2SLS(
            DeltaLogC_n[4:], sm.add_constant(DeltaLogY_n[4:]), instruments
        )
        res_IV = mod_IV.fit()
        ...
        regressors = sm.add_constant(
            np.transpose(np.array([DeltaLogC_predict, DeltaLogY_predict, A_predict]))
        )
        ...
        mod_IV = smsrg.IV2SLS(
            DeltaLogC_n[4:],
            sm.add_constant(
                np.transpose(np.array([DeltaLogC_n[3:-1], DeltaLogY_n[4:], A_n[3:-1]]))
            ),
            instruments,
        )
        res_IV = mod_IV.fit()
        ...
        PvalArray[
            n, 4
        ] = np.nan  # Need to put in KP stat here, may have to do this in Stata
    ...
    panel_text = makeResultsPanel(
        Coeffs=np.mean(CoeffsArray, axis=0),
...
def runStickyEregressionsInStata(
    infile_name, interval_size, meas_err, sticky, all_specs, stata_exe
):
    """
    ...
    # Make results table and return it
    panel_text = makeResultsPanel(
        Coeffs=stata_output.CoeffsArray,
...
def makeResultsPanel(
    Coeffs, StdErrs, Rsq, Pvals, OID, Counts, meas_err, sticky, all_specs
):
    """
    ...
    if all_specs:
        memo += (
            "\\\\ \multicolumn{6}{l}{Memo: For instruments $\mathbf{Z}_{t}$, "
            + DeltaLogC
            + " $= \mathbf{Z}_{t} \zeta, ~\\bar{R}^{2}=$ "
            + mystr1(Counts[3])
        )
        if meas_err:
    ...
    if all_specs:
        ...
        output += (
            "\\\\ "
            + mystr1(Coeffs[4])
            + sigFunc(Coeffs[4], StdErrs[4])
            + " & "
            + mystr1(Coeffs[5])
            + sigFunc(Coeffs[5], StdErrs[5])
            + " & "
            + mystr2(Coeffs[6])
            + sigFunc(Coeffs[6], StdErrs[6])
            + " & IV & "
            + mystr1(Rsq[4])
            + " & "
            + mystr1(Pvals[4])
    ...
    if Counts[0] is not None and Counts[2] > 1 and False:
        output += (
            "\\\\ \multicolumn{6}{c}{Horserace coefficient on "
            + DeltaLogC
        ...
        output += (
            "\\\\ \multicolumn{6}{c}{Horserace coefficient on $\mathbb{E}[\Delta \log \mathbf{Y}_{t+1}]$ significant at 95\% level for "
            + str(Counts[1])
            + " of "
            + str(Counts[2])
            + " subintervals.} \n"
...
def makeResultsTable(caption, panels, counts, filename, label):
    ...
    if counts[1] > 1:
        note += (
            "Reported statistics are the average values for "
            + str(counts[1])
            + " samples of "
            + str(counts[0])
            + " simulated quarters each.  "
        )
        ...
        note += (
            "Reported statistics are for a single simulation of "
            + str(counts[0])
            + " quarters.  "
        )
...
def makeParameterTable(filename, params):
    ...
    macro_panel += (
        "\\\\ $\\kapShare$ & "
        + "{:.2f}".format(params.CapShare)
        + " & Capital's Share of Income   \n"
    )
    macro_panel += (
        "\\\\ $\\daleth$ & "
        + "{:.2f}".format(params.DeprFacAnn)
        + "^{1/4} & Depreciation Factor   \n"
    )
    macro_panel += (
        "\\\\ $\sigma_{\Theta}^{2}$ & "
        + "{:.5f}".format(params.TranShkAggVar)
        + " & Variance Aggregate Transitory Shocks \n"
    )
    macro_panel += (
        "\\\\ $\sigma_{\Psi}^{2}$ & "
        + "{:.5f}".format(params.PermShkAggVar)
    ...
    SS_panel += (
        "\\\\ $\\breve{K}/\\breve{K}^{\\kapShare}$ & "
        + "{:.1f}".format(params.KYratioSS)
        + " & SS Capital to Output Ratio  \n"
    )
    SS_panel += (
        "\\\\ $\\breve{K}$ & "
        + "{:.2f}".format(params.KSS)
        + " & SS Capital to Labor Productivity Ratio ($=12^{1/(1-\\kapShare)}$) \n"
    )
    SS_panel += (
        "\\\\ $\\breve{\\Wage}$ &  "
        + "{:.2f}".format(params.wRteSS)
        + " & SS Wage Rate ($=(1-\\kapShare)\\breve{K}^{\\kapShare}$) \n"
    )
    SS_panel += (
        "\\\\ $\\breve{\\mathsf{r}}$ & "
        + "{:.2f}".format(params.rFreeSS)
        + " & SS Interest Rate ($=\\kapShare \\breve{K}^{\\kapShare-1}$) \n"
    )
    SS_panel += (
        "\\\\ $\\breve{\\Rprod}$ & "
        + "{:.3f}".format(params.RfreeSS)
        + "& SS Between-Period Return Factor ($=\\daleth + \\breve{\\mathsf{r}}$) \n"
    )
    ...
    pref_panel += (
        "\\\\ $\\rho$ & "
        + "{:.0f}".format(params.CRRA)
        + ". & Coefficient of Relative Risk Aversion \n"
    )
    pref_panel += (
        "\\\\ $\\beta_{SOE}$ &  "
        + "{:.3f}".format(params.DiscFacSOE)
        + " & SOE Discount Factor \n"
    )  # ($=0.99 \\cdot \\PLives / (\\breve{\\mathcal{R}} \\Ex [\\pmb{\\psi}^{-\CRRA}])$)\n"
    pref_panel += (
        "\\\\ $\\beta_{DSGE}$ &  "
        + "{:.3f}".format(params.DiscFacDSGE)
        + " & HA-DSGE Discount Factor ($=\\breve{\\Rprod}^{-1}$) \n"
    )
    pref_panel += (
        "\\\\ $\Pi$                    & "
        + "{:.2f}".format(params.UpdatePrb)
    ...
    idio_panel += (
        "\\\\ $\sigma_{\\theta}^{2}$    & "
        + "{:.3f}".format(params.TranShkVar)
        + "     & Variance Idiosyncratic Tran Shocks (=$4 \\times$ Annual) \n"
    )
    idio_panel += (
        "\\\\ $\sigma_{\psi}^{2}$      &"
        + "{:.3f}".format(params.PermShkVar)
        + "      & Variance Idiosyncratic Perm Shocks (=$\\frac{1}{4} \\times$ Annual) \n"
    )
    idio_panel += (
        "\\\\ $\wp$                    & "
        + "{:.3f}".format(params.UnempPrb)
        + "  & Probability of Unemployment Spell \n"
    )
    idio_panel += (
        "\\\\ $\PDies$             & "
        + "{:.3f}".format(params.DiePrb)
...
def makeEquilibriumTable(out_filename, four_in_files, CRRA):
    ...
    SOEfrictionless = np.genfromtxt(
        results_dir + four_in_files[0] + "Results.csv", delimiter=","
    )
    SOEsticky = np.genfromtxt(
        results_dir + four_in_files[1] + "Results.csv", delimiter=","
    )
    DSGEfrictionless = np.genfromtxt(
        results_dir + four_in_files[2] + "Results.csv", delimiter=","
    )
    DSGEsticky = np.genfromtxt(
        results_dir + four_in_files[3] + "Results.csv", delimiter=","
    )

    # Read in value at birth from the four files
    vBirth_SOE_F = np.genfromtxt(
        results_dir + four_in_files[0] + "BirthValue.csv", delimiter=","
    )
    vBirth_SOE_S = np.genfromtxt(
        results_dir + four_in_files[1] + "BirthValue.csv", delimiter=","
    )
    vBirth_DSGE_F = np.genfromtxt(
        results_dir + four_in_files[2] + "BirthValue.csv", delimiter=","
    )
    vBirth_DSGE_S = np.genfromtxt(
        results_dir + four_in_files[3] + "BirthValue.csv", delimiter=","
    )

    # Calculate the cost of stickiness in the SOE and DSGE models
    StickyCost_SOE = np.mean(
        1.0 - (vBirth_SOE_S / vBirth_SOE_F) ** (1.0 / (1.0 - CRRA))
    )
    StickyCost_DSGE = np.mean(
        1.0 - (vBirth_DSGE_S / vBirth_DSGE_F) ** (1.0 / (1.0 - CRRA))
    )
    ...
    main_table += "%\\\\  & & $K$  \n"
    main_table += (
        "\\\\  & & $A$ & {:.2f}".format(SOEfrictionless[0])
        + " &{:.2f}".format(SOEsticky[0])
        + " & {:.2f}".format(DSGEfrictionless[0])
        + " & {:.2f}".format(DSGEsticky[0])
        + "   \n"
    )
    main_table += (
        "\\\\  & & $C$ & {:.2f}".format(SOEfrictionless[1])
        + " &{:.2f}".format(SOEsticky[1])
        + " & {:.2f}".format(DSGEfrictionless[1])
        + " & {:.2f}".format(DSGEsticky[1])
        + "   \n"
    )
    main_table += "\\\\ \\midrule  \n"
    ...
    main_table += (
        "\\\\ & & $\log A $         & {:.3f}".format(SOEfrictionless[2])
        + " & {:.3f}".format(SOEsticky[2])
        + " & {:.3f}".format(DSGEfrictionless[2])
        + " & {:.3f}".format(DSGEsticky[2])
        + " \n"
    )
    main_table += (
        "\\\\ & & $\Delta \log \\CLevBF $  & {:.3f}".format(SOEfrictionless[3])
        + " & {:.3f}".format(SOEsticky[3])
        + " & {:.3f}".format(DSGEfrictionless[3])
        + " & {:.3f}".format(DSGEsticky[3])
        + " \n"
    )
    main_table += (
        "\\\\ & & $\Delta \log \\YLevBF $  & {:.3f}".format(SOEfrictionless[4])
        + " & {:.3f}".format(SOEsticky[4])
        + " & {:.3f}".format(DSGEfrictionless[4])
        + " & {:.3f}".format(DSGEsticky[4])
        + " \n"
    )
    main_table += (
    ...
    main_table += (
        "\\\\ & & $\log \\aLevBF $  & {:.3f}".format(SOEfrictionless[6])
        + " & {:.3f}".format(SOEsticky[6])
        + " & {:.3f}".format(DSGEfrictionless[6])
        + " & {:.3f}".format(DSGEsticky[6])
        + " \n"
    )
    main_table += (
        "\\\\ & & $\log \\cLevBF $  & {:.3f}".format(SOEfrictionless[7])
        + " & {:.3f}".format(SOEsticky[7])
        + " & {:.3f}".format(DSGEfrictionless[7])
        + " & {:.3f}".format(DSGEsticky[7])
        + " \n"
    )
    main_table += (
        "\\\\ & & $\log p $  & {:.3f}".format(SOEfrictionless[8])
        + " & {:.3f}".format(SOEsticky[8])
        + " & {:.3f}".format(DSGEfrictionless[8])
        + " & {:.3f}".format(DSGEsticky[8])
        + " \n"
    )
    main_table += (
        "\\\\ & & $\log \\yLevBF | \\yLevBF > 0 $  & {:.3f}".format(SOEfrictionless[9])
        + " & {:.3f}".format(SOEsticky[9])
        + " & {:.3f}".format(DSGEfrictionless[9])
        + " & {:.3f}".format(DSGEsticky[9])
        + " \n"
    )
    main_table += (
        "\\\\ & & $\Delta \log \\cLevBF $  & {:.3f}".format(SOEfrictionless[11])
        + " & {:.3f}".format(SOEsticky[11])
        + " & {:.3f}".format(DSGEfrictionless[11])
        + " & {:.3f}".format(DSGEsticky[11])
        + " \n"
    )
    main_table += "  \n"
    main_table += "  \n"
...
def extractSampleMicroData(Economy, num_periods, AgentCount, ignore_periods):
    ...
    agg_trans_shk_matrix = deepcopy(
        Economy.TranShkAggNow_hist[ignore_periods : ignore_periods + num_periods]
    )
    wRte_matrix = deepcopy(
        Economy.wRteNow_hist[ignore_periods : ignore_periods + num_periods]
    )
    ...
    c_matrix = deepcopy(
        agent.cLvlNow_hist[
            (ignore_periods + 1) : ignore_periods + num_periods + 1, 0:AgentCount
        ]
    )
    y_matrix = deepcopy(
        agent.yLvlNow_hist[
    ...
    total_trans_shk_matrix = deepcopy(
        agent.TranShkNow_hist[
            (ignore_periods + 1) : ignore_periods + num_periods + 1, 0:AgentCount
        ]
    )
    trans_shk_matrix = (
        total_trans_shk_matrix
        / (np.array(agg_trans_shk_matrix) * np.array(wRte_matrix))[:, None]
    )
    a_matrix = deepcopy(
        agent.aLvlNow_hist[
            (ignore_periods + 1) : ignore_periods + num_periods + 1, 0:AgentCount
    ...
    top_assets = a_matrix_nrm > np.transpose(
        np.tile(np.percentile(a_matrix_nrm, 1, axis=1), (np.shape(a_matrix_nrm)[1], 1))
    )
...
def makeMicroRegressionTable(out_filename, micro_data):
    ...
    for i in range(2):
        ...
        mod = sm.OLS(
            logc_diff[1:],
            sm.add_constant(np.transpose(np.vstack([logc_diff[0:-1]]))),
            missing="drop",
        )
        res = mod.fit()
        ...
        mod = sm.OLS(
            logc_diff[1:],
            sm.add_constant(np.transpose(np.vstack([-log_trans_shk[0:-1]]))),
            missing="drop",
        )
        res = mod.fit()
        ...
        mod = sm.OLS(
            logc_diff[1:],
            sm.add_constant(np.transpose(np.vstack([top_assets[0:-1]]))),
            missing="drop",
        )
        res = mod.fit()
        ...
        mod = sm.OLS(
            ...
            sm.add_constant(
                np.transpose(
                    np.vstack([logc_diff[0:-1], -log_trans_shk[0:-1], top_assets[0:-1]])
                )
            ),
            missing="drop",
        )
    ...
    F_panel += (
        "\\\\ &  {:.3f}".format(coeffs[3, 0])
        + "  &    {:.3f}".format(coeffs[4, 0])
        + "    &     {:.3f}".format(coeffs[5, 0])
        + "   & {:.3f}".format(r_sq[3, 0])
        + " &    \n"
    )
    F_panel += (
    ...
    S_panel += (
        "\\\\ &  {:.3f}".format(coeffs[3, 1])
        + "  &    {:.3f}".format(coeffs[4, 1])
        + "    &     {:.3f}".format(coeffs[5, 1])
        + "   & {:.3f}".format(r_sq[3, 1])
        + " &    \n"
    )
    S_panel += (
...
def makeuCostVsPiFig(uCost_filename):
    ...
    plt.plot(
        [UpdatePrbBase, UpdatePrbBase], [0.0, f(UpdatePrbBase)], "--k"
    )  # Add dashed line at Pi=0.25
    plt.xlim([0.05, 1.0])
    plt.ylim([0.0, 30.0])
    ...
    # Plot uCost vs 1/Pi
    plt.plot(1.0 / UpdatePrbVec, uCostVec * 10000, color="#1f77b4")
    plt.plot(
        [1.0 / UpdatePrbBase, 1.0 / UpdatePrbBase], [0.0, f(UpdatePrbBase)], "--k"
    )  # Add dashed line at Pi=0.25
    plt.xlim([1.0, 16.0])
    plt.ylim([0.0, 35.0])
...
def makeValueVsAggShkVarFig(value_filename):
    ...
    plt.plot(
        [PermShkAggVarBase * 10 ** 5, PermShkAggVarBase * 10 ** 5],
        [vBot, f(PermShkAggVarBase * 10 ** 5)],
        "--k",
    )
    plt.ylim(vBot, vTop)
...
def makeValueVsPiFig(value_filename):
    ...
    vBot = f(1.0 / 16)
    vTop = np.max(vVec)
    plt.plot(1.0 / UpdatePrbVec, vVec, color="#1f77b4")
    plt.plot(
        [1.0 / UpdatePrbBase, 1.0 / UpdatePrbBase], [vBot, f(UpdatePrbBase)], "--k"
    )
    plt.xlim([1.0, 16.0])
    plt.ylim(vBot, vTop)
    plt.xlabel("Expected periods between information updates $\Pi^{-1}$")
