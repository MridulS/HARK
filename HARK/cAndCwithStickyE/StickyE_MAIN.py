...
calc_micro_stats = (
    False
)  # Whether to calculate microeconomic statistics (only matters when run_models is True)
...
save_data = (
    False
)  # Whether to save data for use in Stata (as a tab-delimited text file)
run_ucost_vs_pi = (
    False
)  # Whether to run an exercise that finds the cost of stickiness as it varies with update probability
run_value_vs_aggvar = (
    False
)  # Whether to run an exercise to find value at birth vs variance of aggregate permanent shocks
...
def main():
    ...
    if do_SOE:
        ...
        if make_tables:
            ...
            makeResultsTable(
                None, [frictionless_me_panel], my_counts, "SOEmrkvSimRegF", "tPESOEsimF"
            )
            makeResultsTable(
        ...
        if run_models:
            ...
            if calc_micro_stats:
                StickyDSGEmarkovEconomy.overwrite_hist = (
                    True
                )  # History will be overwritten by sticky outcomes
