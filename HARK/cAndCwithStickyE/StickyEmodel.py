...
from HARK.ConsumptionSaving.RepAgentModel import (
...
class StickyErepAgent(RepAgentConsumerType):
    ...
    def getShocks(self):
        super(
            self.__class__, self
        ).getShocks()  # Get actual permanent and transitory shocks
    ...
    def getStates(self):
        ...
        self.yNrmTrue = (
            self.kNrmTrue ** self.CapShare * self.TranShkNow ** (1.0 - self.CapShare)
            - self.kNrmTrue * self.DeprFac
        )
        self.Rfree = (
            1.0
            + self.CapShare
            * self.kNrmTrue ** (self.CapShare - 1.0)
            * self.TranShkNow ** (1.0 - self.CapShare)
            - self.DeprFac
        )
        self.wRte = (
            (1.0 - self.CapShare)
            * self.kNrmTrue ** self.CapShare
    ...
    def getpLvlPcvd(self):
        ...
        """
        pLvlPcvd = self.UpdatePrb * self.pLvlTrue + (1.0 - self.UpdatePrb) * (
            self.pLvlNow * self.PermGroFac[self.t_cycle[0] - 1]
        )
        return pLvlPcvd
...
class StickyEmarkovRepAgent(RepAgentMarkovConsumerType, StickyErepAgent):
    ...
    def simBirth(self, which_agents):
        ...
        if which_agents == np.array([True]):
            ...
            if self.t_sim == 0:  # Initialize perception distribution for Markov state
                ...
                self.MrkvPcvd[
                    self.MrkvNow[0]
                ] = 1.0  # Correct perception of state initially
    ...
    def getpLvlPcvd(self):
        ...
        dont_mass = self.MrkvPcvd * (1.0 - self.UpdatePrb)  # pmf of non-updaters
        update_mass = np.zeros(StateCount)
        update_mass[i] = self.UpdatePrb  # pmf of updaters

        dont_pLvlPcvd = (
            self.pLvlNow * self.PermGroFac[t - 1]
        )  # those that don't update think pLvl grows at PermGroFac for last observed state
        ...
        pLvlPcvd = (
            dont_mass * dont_pLvlPcvd + update_mass * update_pLvlPcvd
        ) / self.MrkvPcvd
        pLvlPcvd[
            self.MrkvPcvd == 0.0
        ] = 1.0  # Fix division by zero problem when MrkvPcvd[i]=0
        return pLvlPcvd
    ...
    def getControls(self):
        ...
        self.cLvlNow = np.dot(
            cNrmNow * self.pLvlNow, self.MrkvPcvd
        )  # Take average of cLvl across states
