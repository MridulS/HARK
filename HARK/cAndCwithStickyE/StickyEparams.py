...
PermShkVar = PermShkVarAnn / 4.0  # Variance of idiosyncratic permanent shocks
...
periods_to_sim = (
    21010
)  # Total number of periods to simulate; this might be increased by DSGEmarkov model
ignore_periods = (
    1000
)  # Number of simulated periods to ignore (in order to ensure we are near steady state)
...
DiscFacMeanSOE = (
    DiscFacSOE
)  # Central value of intertemporal discount factor for SOE model
...
PermGroFacMin = (
    0.9925
)  # Minimum value of aggregate permanent growth in Markov specifications
PermGroFacMax = (
    1.0075
)  # Maximum value of aggregate permanent growth in Markov specifications
Persistence = (
    0.5
)  # Base probability that macroeconomic Markov state stays the same; else moves up or down by 1
RegimeChangePrb = (
    0.00
)  # Probability of "regime change", randomly jumping to any Markov state
...
DiscFacSetSOE = approxUniform(
    N=TypeCount, bot=DiscFacMeanSOE - DiscFacSpread, top=DiscFacMeanSOE + DiscFacSpread
)[1]
DiscFacSetDSGE = approxUniform(
    N=TypeCount,
    bot=DiscFacMeanDSGE - DiscFacSpread,
    top=DiscFacMeanDSGE + DiscFacSpread,
)[1]
...
init_DSGE_consumer["MgridBase"] = np.array(
    [
        0.1,
        0.3,
        0.5,
