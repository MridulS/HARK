...
class cstwMPCagent(EstimationAgentClass):
    ...
    def updateIncomeProcess(self):
        ...
        if self.cycles == 0:
            ...
            TranShkDstn[0] = np.insert(
                TranShkDstn[0] * (1.0 - self.UnempPrb), 0, self.UnempPrb
            )
            TranShkDstn[1] = np.insert(
                TranShkDstn[1] * (1.0 - tax_rate) * self.IndL, 0, self.IncUnemp
            )
    ...
    def distributeParams(self, param_name, param_count, center, spread, dist_type):
        ...
        while j < len(self.agents):
            for n in range(replication_factor):
                self.agents[j](
                    AgentCount=int(
                        self.Population * param_dist[0][b] * self.TypeWeight[n]
                    )
                )
