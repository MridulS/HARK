...
for b in range(17):
    ...
    plt.fill_between(
        m, np.zeros(m.shape), scale * pdf_array[b,], facecolor="c", alpha=0.5
    )
...
plt.fill_between(
    m, np.zeros(m.shape), scale * 2 * pdf_array[12,], facecolor="c", alpha=0.5
)
...
plt.fill_between(
    m, np.zeros(m.shape), scale * 0.33 * pdf_array[2,], facecolor="c", alpha=0.5
)
