...
class AgentType(HARKobject):
    ...
    def solve(self, verbose=False):
        ...
        with np.errstate(
            ...
            if (
                self.time_flow
            ):  # Put the solution in chronological order if this instance's time flow runs that way
    ...
    def simOnePeriod(self):
        ...
        if not hasattr(self, "solution"):
            raise Exception(
                "Model instance does not have a solution stored. To simulate, it is necessary to run the `solve()` method of the class first."
            )
...
def main():
    ...
    print(
        "To see some examples of its frameworks in action, try running a model module."
    )
    print("Several interesting model modules can be found in /ConsumptionSavingModel.")
    print(
        'For an extraordinarily simple model that demonstrates the "microeconomic" and'
    )
