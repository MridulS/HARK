...
def approxLognormal(
    N, mu=0.0, sigma=1.0, tail_N=0, tail_bound=[0.02, 0.98], tail_order=np.e
):
    """
    ...
    if sigma > 0.0:
        ...
        temp_cutoffs = list(
            stats.lognorm.ppf(CDF_vals[1:-1], s=sigma, loc=0, scale=np.exp(mu))
        )
...
def combineIndepDstns(*distributions):
    ...
    for dd, dist in enumerate(distributions):
        ...
        dist_newshape = (
            (1,) * dd + (len(dist[0]),) + (1,) * (number_of_distributions - dd)
        )
